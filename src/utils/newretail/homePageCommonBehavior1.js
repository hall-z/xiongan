...
export default {
    ...
    onUnmounted: function () {
        ...
        app.globalData.payCodeOrder = {
            count: 0,
            component: null
        };
        app.globalData.cacheOrder = {
            count: 0,
            component: null
        };
    },
    onMounted: function () {
        ...
        this.setData({
            ...
            themeColor:
                app && app.globalData && app.globalData.uiconfig && app.globalData.uiconfig.themeColor ? app.globalData.uiconfig.themeColor : uni.getStorageSync('themeColor')
        });
    ...
    methods: {
                ...
                .then((res) => {
                    ...
                    if (!res.member) {
                        ...
                        if (app.globalData.storeInfo) {
                            that.recordVisitStore(app.globalData.storeInfo.id);
                        }
                        uni.setStorage({
                            key: 'wj_member',
                            data: res.member
        ...
        getMbrCode: function () {
            const that = this;
            if (!this.uiconfig && app.globalData.uiconfig) {
                this.setData({
                    uiconfig: app.globalData.uiconfig
                });
            }
        ...
        clickFunctionEntry: function (e) {
            console.log(e);
            let linkmodel = e.currentTarget.dataset.linkmodel;
            ...
            if (linkmodel == 'scancode_buy') {
                ...
                this.toGiftCard();
            } else if (linkmodel === 'grab_activity' || linkmodel === 'teambuying_activity' || linkmodel === 'product_menu' || linkmodel === 'advance_sell') {
                navigateTo(route);
        ...
        handleIndexPopup: function () {
            ...
            if (!app.globalData.isOfficialAccountQueryLoadCount || !that.customData.hasOfficialAccountQuery || that.show.official) {
                return;
            }
            // 微信已经不允许强制用户授权
            // 判断是否开启极速登录
        ...
        toGoodsDetail: function (val) {
            ...
            let opts = '?productId=' + val.currentTarget.dataset.productid + '&storeId=' + this.storeId + '&type=' + val.currentTarget.dataset.type;
        ...
        activityJumping: function (e) {
            ...
            if (e.currentTarget.dataset.item.grabActivityIdList && e.currentTarget.dataset.item.grabActivityIdList.length > 0) {
                that.toAdPage(advertisementInfo, 'activity', JSON.stringify(e.currentTarget.dataset.item.grabActivityIdList));
            } else {
        ...
        activityJumping2: function (e) {
            const that = this;
            if (e.currentTarget.dataset.item.grabActivityIdList && e.currentTarget.dataset.item.grabActivityIdList.length > 0) {
                that.toAdPage(e.currentTarget.dataset.item, 'activity', JSON.stringify(e.currentTarget.dataset.item.grabActivityIdList));
            } else {
        ...
        toAdPage: function (pageInfo, pageType, data) {
            ...
            if (pageInfo.linkType === 'APP') {
                ...
                that.verifyGoods(app.globalData.storeInfo.id, pageInfo.productId, 'normal');
                // 商品
        ...
        verifyGoods(storeId, productId, type) {
            productService
                ...
                .then((res) => {
                    if (res) {
                        // 商品
                        let opts = '?productId=' + productId + '&storeId=' + app.globalData.storeInfo.id + '&type=' + type;
                        NAVPAGE.toGoodsDetails(opts);
                    }
                })
                .catch((err) => {
        ...
        toProductDetail: function (e) {
            ...
            let n = 0;
            for (let t in e) {
                opts = 0 === n ? opts + '?' + t + '=' + e[t] : opts + '&' + t + '=' + e[t];
                n++;
            }
            NAVPAGE.toGoodsDetails(opts);
        },
        ...
        addToCart: function (e) {
            ...
            this.i = i;
            this.setData({
                initialpurchasenumber: e.detail.detail.target.dataset.initialpurchasenumber || null
            });
        ...
        addShopcart: function (val) {
            ...
            if (val.detail.target.dataset.grabtype && val.detail.target.dataset.grabtype === 'SPEC') {
                // 多规格秒杀商品跳转商品详情
                let grabActivityId = val.detail.target.dataset.grabactivityid;
                let opts = '?productId=' + val.detail.target.dataset.id + '&storeId=' + (val.detail.target.dataset.storeid || this.storeId) + '&source=addshopcart';
        ...
        handleAddShopCart: function (val) {
            ...
            let balance = val.detail.target.dataset.balance;
            let grabActivityId = val.detail.target.dataset.grabactivityid;
            let initialpurchasenumber = val.detail.target.dataset.initialpurchasenumber;
            this.setData({
            ...
            if (style !== 'SPEC_PARENT' && style !== 'MEALS2') {
                if (loginStatus) {
                    if (business === 'DISTRIBUTION') {
                        try {
                            var data = uni.getStorageSync('wj_distributionStore');
                            if (data) {
                                storeId = data.id;
                            }
                        } catch (e) {
                      ...
                      storeId = val.detail.target.dataset.storeid || app.globalData.storeInfo.id
                    }
                    var postData = {
                        storeId: storeId,
                        lists: [
                    ...
                    var localShopingCart = uni.getStorageSync('wj_shopCartStorage');
                    if (business === 'DISTRIBUTION') {
                        balance = 0;
                        productService
                            .getDetails(storeId, productId)
                            .then((res) => {
                                let goodNum = null;
                                if (localShopingCart && localShopingCart.goodsId && localShopingCart.goodsId.indexOf(productId) > -1) {
                                ...
                                shopcartService
                                    ...
                                    .then((res) => {
                                        ...
                                        if (mode === 'reduce') {
                                            ...
                                            let grabProducts = that.grabProducts;
                                            if (that.grabProducts[productId] != null) {
                                                grabProducts[productId] = parseInt(that.grabProducts[productId]) + (that.initialpurchasenumber || 1);
                                            } else {
        ...
        getLoginDonateCouponAdsense: function (storeId) {
            ...
            if (this.onPullDownRefresh === false) {
                try {
                    var value = uni.getStorageSync('wj_loginDonateCouponCount');
                    if (value) {
                        let tempTime = value.replace(/-/g, '/');
                        let nowTime = new UTILS.ResponseDate().getTime();
        ...
        clickAssignCoupon: function (e) {
            const that = this;
            if (app.globalData.userInfo && app.globalData.userInfo.member) {
                let coupons = this.donateCoupons;
                ...
                let tempCouponsArr = coupons.donateCouponActivityIds.split(',');
                let couponsArr = [];
        ...
        getluckDraw: function (storeId) {
            ...
            if (this.onPullDownRefresh === false) {
                try {
                    var value = uni.getStorageSync('wj_luckyDrawRecordTime');
                    if (value) {
                        let tempTime = value.replace(/-/g, '/');
                        let nowTime = new UTILS.ResponseDate().getTime();
                        let time = new Date(tempTime).getTime();
        ...
        handleLuckDrawPopupClose(val) {
            const that = this;
            let raffleInfo = this.raffleInfo;
            var raffleInfoArr = raffleInfo ? Object.keys(raffleInfo) : 0;
            if (val != null) {
                this.setData({
                    'show.luckDraw': false
                });
        ...
        addLocalGoods: function (id) {
            try {
                // 读取缓存的商品总量和购物车数据
                var localShopingCart = uni.getStorageSync('wj_shopCartStorage');
                var localProductCount = uni.getStorageSync('wj_userProductsCount');
                // 设置购物车中商品总数量
                ...
                if (localShopingCart) {
                    ...
                    if (index > -1) {
                        ...
                        tempShopCart[_id] =
                            ...
                            (this.initialpurchasenumber && this.initialpurchasenumber > Number(tempShopCart[_id]) ? this.initialpurchasenumber - Number(tempShopCart[_id]) : 1);
                    } else {
                        //新增
        ...
        reduceLocalGoods: function (id) {
            try {
                ...
                if (localShopingCart) {
                    ...
                    if (index > -1) {
                        //已经存在购物车，只是数量变化
                        if (this.initialpurchasenumber && Number(tempShopCart[_id]) <= this.initialpurchasenumber) {
        ...
        handleTouchOnGoods: function (e) {
            ...
            if (e != null) {
                if (e.target.dataset.addtype === 'normal') {
                    // 首页精选
                    let data = this.fineQualityGoodsList;
                    data.forEach((item) => {
                    ...
                    let data = this.activityList[e.target.dataset.index];
                    ...
                    // 主题活动三分图
                    // e.target.dataset.index 当前点击的商品在主题活动数组中是第几个
                    let data = this.grabData.products;
        ...
        addShopcartAnimation: function (e) {
            const that = this;
            let tempArr = Object.keys(e.detail.animation);
        ...
        checkUserInfo: function () {
            if (app.globalData.userInfo) {
                this.setData({
                    hasUserInfo: true,
                    userInfo: app.globalData.userInfo.wxaUser
                });
                // 判断用户是否是会员
                if (app.globalData.userInfo.member) {
        ...
        renderingPageByUI: function () {
            ...
            if (uiconfig.isHomeShowTopMember && uiconfig.topMemberEntrance != null && uiconfig.topMemberEntrance.length > 0) {
        ...
        getUnVisitedCoupons: function () {
            ...
            couponService
                ...
                .then((res) => {
                    ...
                    //筛选出未读的券
                    let unVisitedCoupons = res['records'].filter((value) => value.visited === false);
                    ...
                    couponList = couponList.concat(cashCoupons, discountCoupons, freeshipCoupons, giftCoupons, otherCoupons);
                    that.setData({
                        unVisitedCoupons: couponList
                    });
                    that.toggleUnvisitedCouponPopup();
                    var ids = unVisitedCoupons.map(function (item) {
                        return item.id;
                    });
        ...
        getCacheOrder: function () {
            const that = this;
            if (app.globalData.cacheOrder.count && app.globalData.cacheOrder.component !== that) {
                console.log('当前已经有查询');
                return;
            }
            app.globalData.cacheOrder = {
                ...
                orderService
                    ...
                    .then((res) => {
                        if (res && res.products.length > 0) {
                                    ...
                                    if (res.productDetails && res.productDetails.length > 0) {
                                        res.productDetails.forEach((ele) => {
                                            if (item.productId === ele.id) {
                                                ...
                                                if (that.isVIP && that.vipGradeConfig.openMemberPrice && ele.memberPrice && Number(ele.memberPrice) < Number(ele.sellPrice)) {
                            ...
                            var orderTimer = setTimeout(() => {
                                console.log('轮询缓存订单');
                                let page = UTILS.utilGetCurrentPage();
        ...
        getMemberGrade: function () {
            ...
            if (app.globalData.userInfo && app.globalData.userInfo.member) {
                ...
                if (app.globalData.memberGrade) {
                    that.setData({
                        memberGrade: app.globalData.memberGrade
                    });
                } else {
                    if (member.gradeId) {
                        memberService
