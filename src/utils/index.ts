...
export function isUnder16(idCard) {
  // 检查身份证号码长度
  // 验证身份证号长度
  if (idCard.length !== 18) {
...
export function isUnder18(idCard) {
  // 检查身份证号码长度
  // 验证身份证号长度
  if (idCard.length !== 18) {
    throw new Error('身份证号必须是18位')
...
export function isValidAge(age) {
  const basicAddressRegex = /^([1-9]|[1-9][0-9])$/
  return basicAddressRegex.test(age)
}
export const testUserName = (name) => {
  return /^(?=[a-z0-9A-Z\u4e00-\u9fa5·]{2,10}$)[a-z0-9A-Z\u4e00-\u9fa5]+·?[a-z0-9A-Z\u4e00-\u9fa5]+$/.test(
    name,
  )
}
...
/**
 * @description:  将文本转换为JSON对象
 * @param {} textareaValue 传入的文本
 * @return {}  JSON对象
 */
export function safeParseWechatPayJSON(input) {
  // 预处理步骤：清理输入数据
