...
onMounted(async () => {
  ...
  if (uni.createCameraContext) {
    ...
    message.alert('当前微信版本过低，无法使用该功能，请升级到最新微信版本后重试。')
  }
...
const chooseImage = () => {
  uni.chooseImage({
    ...
    success: async (res) => {
      // 根据比例截图
      const cutUrl = await cutAlbumImage(res.tempFilePaths[0])

      // 如果是头像上传
      if (currData.value.imgType === 0) {
        copSrc.value = cutUrl
      } else {
        // 如果是其它类型，需要对图片进行顺时针90度翻转,用于拍照页面的回显
        const rotateUrl = await rotateAlbumImage(cutUrl)
...
<template>
  <view class="bigBox">
    <view class="box">
      ...
      <view class="cover-box flex flex-col justify-center items-center">
        <view v-if="!copSrc" class="flex flex-col justify-center items-center">
          ...
          <cover-image
            ref="cover"
            v-if="currData.imgType === 5 || currData.imgType === 6"
            class="w-350px h-474px"
            :src="card3"
            id="cover"
          />
      ...
      <view class="w-full h-10% absolute bottom-0 bg-#000 font-size-20px color-#fff z-99">
        <view class="flex justify-between items-center px-70px py-10px">
          <view class="back backIcon" @click="close">
            <wd-icon name="thin-arrow-left" size="14px" color="#000"></wd-icon>
          </view>
    ...
    <view class="fixed left-9999">
      <canvas
        id="targetSignature"
        type="2d"
        :style="`width:${currData.height}px; height:${currData.width}px;`"
      ></canvas>
    </view>
  </view>
</template>
