  ...
  <view class="content">
    <view class="user-info">
      ...
      <view class="name-mobile">
        ...
        <text>{{state.member.id ? filters.filtMobile(state.member.mobile) : "xxx xxxx xxxx"}}</text>
      </view>
    </view>
...
<view class="order-info" v-if="!state.loading">
  ...
  <view class="order-details">
    <view class="goods-list" :style="state.showMoreGoods ? 'min-height: 102rpx;' : 'max-height: 362rpx;'">
    ...
    <view class="more-details" @click="showGoodsDeatils">
      ...
      <image :src="state.imagesPath.iconFacePayMore" :class="state.showMoreGoods ? 'show' : ''"></image>
    </view>
  </view>
</view>
<view class="discount-info" v-if="!state.loading">
  <view class="discount-item" @click="handleScoreChoice" data-type="discount">
    <view class="left" :data-scorerule="state.scoreInfo.scoreRule" @click.stop="viewScoreWarn">积分抵扣
      <image src="http://xianfengapp.oss-cn-hangzhou.aliyuncs.com/xianfengImages/iconOrderTips2.png"></image>
    </view>
    <view class="right">最多可抵
  ...
  <view class="discount-item" @click="toChoiceCoupon" data-type="discount">
    ...
    <view class="right">
      <text class="no-coupon" v-if="state.couponDiscount === 0">{{discountNum > 0 ? discountNum + '张可用优惠券' : '暂无可用优惠券'}}</text>
      <text class="amount" v-else>-￥<text class="now-price">{{state.couponDiscount}}</text></text>
...
<view class="pay-info" v-if="isMember && !state.loading">
  <view class="pay-item card" data-type="CARD" @click="handlePayMethod">
    ...
    <image :src="state.payMethod.useStoredValueCard ? state.imagesPath.iconFacePaySelected : state.imagesPath.iconFacePayUnSelect" class="select-icon"></image>
  </view>
  <view class="pay-item wx" data-type="WX" @click="handlePayMethod">
    ...
    <image :src="state.payMethod.facePay ? state.imagesPath.iconFacePaySelected : state.imagesPath.iconFacePayUnSelect" class="select-icon"></image>
  </view>
</view>
<view class="loading-box" v-if="isMember && state.loading">
  <loading></loading>
</view>
<view class="middle-button" v-if="!state.hasUserInfo || !isMember">
  <button class="pay-btn" :style="state.theme.mainBgColor" v-if="!state.hasUserInfo" @click="getUserInfo">获取授权</button>
  <button class="pay-btn" :style="state.theme.mainBgColor" v-else-if="!isMember" open-type="getPhoneNumber" @getphonenumber="getPhoneNumber">手机号授权登录</button>
</view>
<view class="bottom-button" v-if="state.hasUserInfo && isMember">
  <button class="pay-btn" :style="state.theme.mainBgColor" v-if="!state.hasUserInfo" @click="getUserInfo">获取授权</button>
  <button class="pay-btn" :style="state.theme.mainBgColor" v-else-if="!isMember" open-type="getPhoneNumber" @getphonenumber="getPhoneNumber">手机号授权登录</button>
  <button class="pay-btn" :style="state.theme.mainBgColor" v-else-if="state.disabled" :disabled="state.disabled">确认支付</button>
  <button class="pay-btn" :style="state.theme.mainBgColor" v-else :disabled="state.disabled" @click="toPay">确认支付￥{{filters.filtToFix(allPrice - state.totalDiscount - state.scoreDiscount)}}</button>
</view>
<!-- 付款超时弹窗 -->
...
function querySystemOptions() {
  ...
  let keys = ['isGiftCard', 'isBothScoreAndCoupon', 'fictitiousProductId'];
  systemService.query(keys).then(res => {
    if (res && res.length > 0) {
      res.forEach(item => {
        ...
        } else if (item.key === "fictitiousProductId" && item.value != null && item.value !== "") {
...
function login() {
  ...
  wxaUserService.login().then(res => {
    ...
    if (errMsg.indexOf(msg) > -1 || errMsg.indexOf(authorize) > -1 || errMsg.indexOf("fail") > -1) {} else {
      uni.showToast({
        title: errMsg,
        icon: 'none',
        duration: 2000
...
function getPhoneNumber(e) {
  ...
  if (e.detail.errMsg.indexOf("ok") >= 0) {
    ...
    wxaUserService.bindMobileByEncrypt(params).then(res => {
...
function handleCardBalance(isUserChange = false) {
  ...
  let tempPrice = parseFloat((allPrice - totalDiscount - scoreDeductionAmount - giftCardDiscount).toFixed(2));
...
function handleGetCacheOrder(amount, that) {
  ...
  orderService.getCacheOrder(that.data.memberCode).then(res => {
          ...
          if (res.productDetails && res.productDetails.length > 0) {
            res.productDetails.forEach(ele => {
              if (item.productId === ele.id) {
                ...
                if (that.data.isVIP && that.data.vipGradeConfig.openMemberPrice && ele.memberPrice && Number(ele.memberPrice) < Number(ele.sellPrice)) {
...
function getAvailableCouponNum() {
  ...
  function queryAvailableCoupon() {
    let allPrice = parseFloat(data.allPrice);
    if (allPrice > 0) {
      couponService.getAvailableCoupon(null, postData).then(res => {
        ...
        if (!state.couponInfo.useCoupon && state.usedCoupons.length === 0 && tempCoupon && (deductionAmount > 0 || deductionRate < 1)) {
          let couponData = {
            couponType: "discount",
            coupons: [tempCoupon]
          };
...
function calcPromotions() {
  ...
  promotionService.calcPromotions(postData).then(res => {
    ...
    if (res) {
      // 处理运费券
      let shipmentDiscount = res.discounts.shipmentDiscount && res.discounts.shipmentDiscount.details.length > 0 ? res.discounts.shipmentDiscount.discount : state.shipmentDiscount; // 运费抵扣
      // 处理运费券不能使用的情况
      if (state.type === "dispatch" && shipmentDiscount === 0 && state.couponInfo.data.freeship && state.couponInfo.data.freeship.length > 0) {
      ...
      let shipmentRealPay = Number(state.shipmentAmount) - Number(shipmentDiscount) > 0 ? parseFloat((Number(state.shipmentAmount) - Number(shipmentDiscount)).toFixed(2)) : 0;
      // 计算促销(拼团不参与送商品促销)
...
function getPostData() {
  ...
  let paymentPrice = parseFloat(state.allPrice) - parseFloat(state.totalDiscount) - parseFloat(state.scoreDiscount);
  ...
  let receiverMobile = state.member.mobile;
  postData = {
...
function getComposePay(authCode) {
  ...
  orderService.getCashPaySign(postData).then(res => {
    getOrderStatusById(order.id, self);
    let total = parseFloat((parseFloat(order.cashTotal) - parseFloat(state.payMethod.cardDeductTotal)).toFixed(2));
...
function handleScoreChoice() {
  ...
  let paymentPrice = parseFloat(state.allPrice) - parseFloat(state.totalDiscount) + parseFloat(state.couponDiscount) + parseFloat(state.shipmentAmount) - parseFloat(state.shipmentCouponDiscount);
  paymentPrice = paymentPrice > 0 ? paymentPrice.toFixed(2) : 0;
  const tempUseCoupon = state.couponInfo.useCoupon ? true : false;
