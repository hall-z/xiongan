<template>
<!-- pages/choiceCoupon/choiceCoupon.wxml -->
...
<view class="choice-coupon-main" v-if="!state.loading">
  <view class="coupon-box" :hidden="state.allFreeship.length === 0" v-if="state.couponType === 'freeship'">
    <view class="coupon-title">运费券
      <text>(可与其他优惠券叠加使用)</text>
    </view>
    <view v-for="(item , i) in state.freeshipCoupon" :key="id">
      <coupon-item @onClickCouponItem="clickCouponItem" :coupon="item.coupon" :selected="current" :noSelected="item.isNotSelect" couponStatus="enable"></coupon-item>
  ...
  <view class="coupon-box" :hidden="state.allDiscount.length === 0" v-if="state.couponType === 'discount'">
    <view class="coupon-title">优惠券
      <text>(可与其他运费券叠加使用)</text>
    </view>
    <view v-for="(item , i) in state.couponList" :key="id">
      <coupon-item @onClickCouponItem="clickCouponItem" :coupon="item.coupon" :selected="currentItem" couponStatus="enable"></coupon-item>
    </view>
...
function clickCouponItem(val) {
  ...
  data.currentTarget.dataset = val.detail;
  couponClick(data);
}
function couponClick(val) {
...
function toPerfectOrder() {
  let couponId = state.currentItem;
  let freeshipId = state.current;
  let chooseCouponData = [];
  let typeData = JSON.stringify(options.type);
  ...
  couponService.getAvailableCoupon(isQrPay, postData).then(res => {
    ...
    res.forEach(ele => {
      ...
      if (ele.coupon.function === "FREESHIP") {
        if (ele.usable) {
          if (state.notSelectedCoupons && state.notSelectedCoupons.length > 0 && state.notSelectedCoupons.indexOf(ele.coupon.id) > -1) {
            ele.isNotSelect = true;
          }
    ...
    state.currentItem = currentDiscountCoupon;
    state.current = currentFreeshipCoupon;
    if (state.selectedCoupons && state.selectedCoupons.length > 0) {
      state.currentItem = state.selectedCoupons[0];
      state.current = state.selectedCoupons[0];
    }
  }).catch(err => {
    uni.showToast({
      title: err.message,
...
onLoad(function (_options) {
  ...
  state.notSelectedCoupons = uni.getStorageSync('notSelectedCoupons') || [];
...
<style scoped>
/* pages/choiceCoupon/choiceCoupon.wxss */
