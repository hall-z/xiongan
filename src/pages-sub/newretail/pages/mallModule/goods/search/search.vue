...
<template>
  ...
  <view class="search-page">
    ...
    <view
      class="search search-page-box"
      :style="
        state.isSearchFixed
          ? 'position: fixed;z-index:4;top:' + state.navHeight + 'px;'
          : 'position: relative;'
      "
    >
      <view
        class="topHandleView"
        :style="'border-color: ' + state.themeColor"
        @click="handleConfirm"
      >
        <image
          lazy-load=""
          style="width: 26rpx; height: 26rpx"
          :src="state.imagesPath.newSearchIcon"
        ></image>
        <!-- <view>请输入关键词搜索</view> -->
        <input
          confirm-type="search"
          :focus="state.searchFocus"
          :placeholder="state.placeholderText || state.searchText"
    ...
    <view class="history" v-if="state.goodsList.length == 0 && state.storeList.length == 0">
      <view class="title">
        历史记录
      ...
      <text
        class="history-item"
        v-for="(searchHistoryItem, index) in state.searchHistory"
        :key="index"
        :data-value="searchHistoryItem"
    ...
    <view
      class="banner-box"
      v-if="state.banners.length > 0 && state.goodsList.length < 1 && state.storeList.length < 1"
    >
...
<view class="microphone-box" hidden="{{state.goodsList.length > 0}}" style="bottom:{{state.keyboardHeight}}px">
  ...
  按住说我要
    ...
    <scroll-view
      class="goods-box recommend-store"
      scroll-y=""
      @scroll="scrollFn"
    ...
    >
      ...
      >
        ...
        <scroll-view
          ...
          style="width: 100%"
        >
          <view v-for="(item, index) in item.productList" :key="index" class="scroll-product">
            ...
            <view class="gm-goods-price" v-if="item.sellPrice != null">
              <view>
                <text class="nowPrice" :style="'color: ' + state.activityColor">
    ...
    >
      ...
      >
        ...
        <view class="goods-info">
          ...
          <text class="goods-desc" v-if="item.type != 'ADVANCE_SELL'">
            {{ item.description ? item.description : '' }}
          </text>
          <view class="goods-name" v-if="item.type != 'ADVANCE_SELL'">
            <view v-for="(item, index) in item.produtlabel" :key="index" v-show="index < 3">
              <text
          ...
          <view class="labels-board" v-if="item.type != 'ADVANCE_SELL'">
          ...
          <view class="goods-price" style="margin-top: 40rpx" v-if="item.type == 'ADVANCE_SELL'">
            ...
            <view class="price-item">
              预售价
              <text class="nowPrice" v-if="item.sellPrice">
                <text>￥</text>
                {{ item.sellPrice }}
              </text>
            </view>
            <view class="price-item front-money" v-if="item.payType === 'PARTIAL'">
              定金
              <text class="nowPrice" v-if="item.frontPrice">
                <text>￥</text>
                {{ item.frontPrice }}
              </text>
            </view>
          ...
          <view class="goods-price" v-else-if="item.type == 'CYCLE'">
            ￥
            <text class="nowPrice">{{ filtToFix(item.sellPrice || 0) }}</text>
            <text class="rise" v-if="item.style == 'SPEC_PARENT'">起</text>
            <view class="vip-price">
              <text
                class="oldPrice"
          ...
          <view class="goods-price" v-else-if="item.sellPrice && item.sellPrice !== undefined">
            ￥
          ...
          <form
            ...
            v-if="item.type != 'ADVANCE_SELL' && item.type != 'CYCLE'"
          >
            ...
            <text class="cart-count">{{shopCart[item.id] ? shopCart[item.id] : ''}}</text>
              ...
              <view class="add-box" :style="'background-color: ' + state.themeColor + ';'">
                ...
                <text class="cart-count" v-if="state.shopCart[item.id]">
                  {{ state.shopCart[item.id] ? state.shopCart[item.id] : '' }}
                </text>
              </view>
            </view>
          </form>
...
function filtToFix(num) {
  return parseFloat(num) > 0 ? parseFloat(num).toFixed(2) : '0'
}
...
async function getSearchProduct(keywordLikes, nameLikes) {
  ...
  if (keywordLikes.indexOf('，')) {
    const keywords = keywordLikes.split('，')
    keywordLikes = ''
  ...
  if (state.showStore) {
    ...
    Promise.allSettled([
      productService.getSearchProduct(storeIdIn, page, 0, keywordLikes, searchCount, '', nameLikes),
      productService.getSearchActiveProduct(params),
    ])
      .then((res) => {
        const tempList = []
        uni.hideLoading()
...
function toGoodsDetail(val, openFlag) {
  ...
  if (type == 'CYCLE') {
    const opts = '?id=' + val.currentTarget.dataset.activeid
    ...
    const opts =
      ...
      '&advanceId=' +
      val.currentTarget.dataset.activeid +
      '&type=advanceSell'
    NAVPAGE.toGoodsDetails(opts)
  } else {
    if (state.linktypea == 'self') {
...
onLoad(function (_options) {
  ...
  if (_options && _options.shareId && _options.shareId != '' && _options.shareId != undefined) {
