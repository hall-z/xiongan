  ...
  <view class="goods-detail-page">
    ...
    >
      ...
      <view class="swiperContainer" :id="'products-' + state.goodsId">
          ...
          <view v-for="(item, index) in state.productPictures" :key="index">
            <swiper-item class="swiper-item">
              ...
              <video
                ...
                :src="item.url"
                :autoplay="state.isWifi && state.productPictures.length == 1"
                show-mute-btn="true"
        ...
        <view class="imageCount" :style="state.isTeam ? 'bottom: 136rpx;' : ''">
      ...
      <view class="secKill-box group-box" v-if="state.isSecKill && state.secKillInfo">
        ...
        <image
          :src="state.imagesPath.seckill_icon"
          mode=""
          class="cuxiao_icon seckill_icon"
        ></image>
        ...
        <view class="secKill-price-box group-people-number">
          <view class="secKill-price-box group-price-box">
            <text class="secKill-nowPrice group-nowPrice">
              ￥
              <text>{{ state.secKillInfo.price ? state.secKillInfo.price : '' }}</text>
            </text>
            <!-- <text class="price-type">秒杀价</text> -->
          </view>
          <view
            class="secKill-price-right group-price-oldPrice"
            v-if="
              state.secKillInfo.originalPrice > state.secKillInfo.price ||
              state.secKillInfo.sellPrice > state.secKillInfo.price
            "
          >
            <!-- <text class="group-number">限时秒杀、数量有限、先到先得</text> -->
            <text class="secKill-oldPrice group-oldPrice">
              {{
                state.secKillInfo.originalPrice &&
                state.secKillInfo.originalPrice > state.secKillInfo.sellPrice
                  ? '￥' + state.secKillInfo.originalPrice
                  : '￥' + state.secKillInfo.sellPrice
              }}
            </text>
          </view>
        </view>
        <view
          class="secKill-time-down group-time-down"
          v-if="state.secKillInfo.secKillStatus !== 'end'"
        >
          <!-- <image class="secKill-time-down-bg group-time-down-bg" src="{{imagesPath.iconDetailsBlockRight}}" ></image> -->
          <image
            class="secKill-time-down-bg group-time-down-bg"
            :src="state.imagesPath.seckill_right_icon"
          ></image>
          <view class="group-time-down-tips">
            {{ state.secKillInfo.secKillStatus === 'created' ? '距开始还有' : '距结束仅剩' }}
          </view>
      ...
      <view class="secKill-box group-box" v-else-if="state.isTeam">
        ...
        <view class="seckill_label">
          拼团
          <text class="group-number">{{ state.groupNumber }}人团，组团成功享优惠价</text>
        </view>
        <view class="secKill-price-box group-people-number">
          <view class="secKill-price-box group-price-box">
            <text class="secKill-nowPrice group-nowPrice">
              ￥
              <text>{{ state.teamAllPrice }}</text>
            </text>
          </view>
          <view class="secKill-price-right group-price-oldPrice">
            <text
              class="group-oldPrice"
              v-if="state.originalPrice && state.teamLeaderPrice < state.originalPrice"
            >
              ￥{{ state.originalPrice }}
            </text>
          </view>
        ...
        <view class="secKill-time-down group-time-down" v-if="state.groupStatus !== 'FINISHED'">
          <image
            class="team-time-down-bg group-time-down-bg"
            :src="state.imagesPath.team_right_icon"
          ></image>
          <view class="group-time-down-tips team-time-down-tips">
            {{ state.groupStatus === 'UNOPENED' ? '距拼团开始' : '距拼团结束' }}
          </view>
          <view class="group-time-down-show team-time-down-show">
            <text class="last-hour txt">{{ state.groupTime.day }}</text>
            <text class="colon">天</text>
            <text class="last-hour txt">{{ state.groupTime.hou }}</text>
            <text class="colon">:</text>
            <text class="last-minute txt">{{ state.groupTime.min }}</text>
            <text class="colon">:</text>
            <text class="last-second txt">{{ state.groupTime.sec }}</text>
          </view>
        </view>
        <view class="group-time-down" v-else>
          <image class="group-time-down-bg" :src="state.imagesPath.iconGroupRightDetails"></image>
          活动已结束
        </view>
      </view>
      <!-- 商品名称价格等 -->
      <view
        v-else-if="
          state.promotionPrice &&
          (!state.memberPrice ||
            (state.promotionMinSellPrice
              ? state.promotionMinSellPrice < state.memberPrice
              : state.promotionPrice < state.memberPrice))
        "
        class="promotion_price_box"
      >
        ...
        <view class="promotion_label">
          {{ state.currentPage === 'welfarePlan' ? '福利价' : '促销' }}
        </view>
        <!-- <text class="goods-nowPrice" ><text>￥</text>{{promotionPrice}}</text> -->
        <text class="goods-nowPrice">
          <text>￥</text>
          {{
            state.specificationsFlag && state.promotionMinSellPrice != state.promotionMaxSellPrice
              ? filtToFix(state.promotionMinSellPrice) +
                '-' +
                filtToFix(state.promotionMaxSellPrice)
              : filtToFix(state.promotionPrice)
          }}
        </text>
        <text
          class="goods-oldPrice"
          v-if="
            !state.promotionMinSellPrice &&
            (state.promotionPrice < state.sellPrice || state.promotionPrice < state.originalPrice)
          "
        >
          ￥{{ state.sellPrice > state.originalPrice ? state.sellPrice : state.originalPrice }}
        </text>
      </view>
      <!-- 花艺师、服务项目 -->
      <view
        v-else-if="state.goodType === 'service' || state.goodType == 'florist'"
        class="promotion_price_box florist_price_box"
      >
        <view>
          <text class="goods-nowPrice">
            <text>￥</text>
            {{
              state.serviceMinPrice != state.serviceMaxPrice
                ? filtToFix(state.serviceMinPrice) + '-' + filtToFix(state.serviceMaxPrice)
                : filtToFix(state.serviceMaxPrice)
            }}
          </text>
          <!-- <text class="goods-oldPrice" wx:if="{{serviceOriginalSellPrice && serviceMaxPrice < serviceOriginalSellPrice}}">￥{{filtToFix(serviceOriginalSellPrice)}}</text> -->
        </view>
      </view>
      <!-- 预售 -->
      <view v-else-if="state.goodType === 'advanceSell'" class="promotion_price_box">
        ...
        <view class="promotion_price_box1">
          <text class="goods-nowPrice" v-if="state.pdtDetail.sellPrice">
            <text>预售价￥</text>
            {{ filtToFix(state.pdtDetail.sellPrice) }}
          </text>
          <view class="price-item front-money" v-if="state.pdtDetail.payType === 'PARTIAL'">
            定金￥
            <text class="price" v-if="state.pdtDetail.frontPrice">
              {{ filtToFix(state.pdtDetail.frontPrice) }}
            </text>
          </view>
        </view>
      </view>
      <view :class="'goods-info-box ' + (state.isTeam ? 'group' : '')">
        ...
        >
          <view v-if="state.goodType === 'score'">
            <text class="goods-nowPrice" v-if="!state.scoreInfo.amount">
              {{ state.scoreInfo.score }}
              <text>积分</text>
            </text>
            <text class="goods-nowPrice" v-if="state.scoreInfo.amount">
              {{ state.scoreInfo.score + '积分+' + state.scoreInfo.amount + '元' }}
            </text>
          ...
          <!-- <view class="goods-price" wx:if="{{state.goodType === 'advanceSell'}}">
            <view class='price-item'>
              预售价
              <text class="price" hidden="{{state.pdtDetail.sellPrice ? false : true}}">￥{{filtToFix(state.pdtDetail.sellPrice)}}</text>
            </view>
            <view class='price-item front-money' wx:if="{{state.pdtDetail.payType === 'PARTIAL'}}">
              定金
              <text class="price" hidden="{{state.pdtDetail.frontPrice ? false : true}}">￥{{filtToFix(state.pdtDetail.frontPrice)}}</text>
            </view>
          </view> -->
          <view v-else-if="state.promotionPrice != null">
            <text class="goods-nowPrice" v-if="state.promotionPrice < state.memberPrice">
              <text>￥</text>
              {{
                state.specificationsFlag
                  ? filtToFix(state.promotionMinSellPrice) +
                    '-' +
                    filtToFix(state.promotionMaxSellPrice)
                  : filtToFix(state.promotionPrice)
            ...
            <view v-else>
              ...
              <text class="goods-nowPrice">
                ...
                {{
                  state.specificationsFlag &&
                  state.promotionMinSellPrice != state.promotionMaxSellPrice
                    ? filtToFix(state.promotionMinSellPrice) +
                      '-' +
                      filtToFix(state.promotionMaxSellPrice)
                    : filtToFix(state.memberPrice)
                }}
              </text>
            </view>
            <text class="goods-oldPrice">
              {{
                state.sellPrice && state.sellPrice > state.promotionPrice
                  ? '￥' + filtToFix(state.sellPrice)
                  : ''
              }}
            </text>
          </view>
          <view v-else>
            <view
              v-if="
                state.memberPrice &&
                state.memberPrice < state.sellPrice &&
                state.memberPrice < state.sellPrice
              "
            >
              <view class="vip-price" style="margin-right: 10rpx">
            ...
            <view v-else>
              <text
                class="goods-nowPrice"
                v-if="
                  !state.originalPrice ||
                  (state.sellPrice && state.sellPrice && state.sellPrice !== undefined)
                "
              >
                <text>￥</text>
                {{
                  ...
                  state.promotionMinSellPrice != state.promotionMaxSellPrice
                    ? filtToFix(state.promotionMinSellPrice) +
                      '-' +
                      filtToFix(state.promotionMaxSellPrice)
                    : filtToFix(state.minSellPrice || state.sellPrice)
                }}
              </text>
            </view>
            <view
            ...
            >
              <text class="goods-oldPrice">￥{{ filtToFix(state.sellPrice) }}</text>
              <view class="vip-label"></view>
            </view>
            <text
              class="goods-oldPrice"
              v-else-if="state.originalPrice && state.sellPrice < state.originalPrice"
            >
              ￥{{ filtToFix(state.originalPrice) }}
            </text>
          </view>
          <text
            class="share-commission"
            v-if="
              state.systemOptions.hasDistributionCenter &&
              state.isDistributor &&
              state.commissionTotal > 0
            "
          >
            分享可得佣金: ￥{{ filtToFix(state.commissionTotal) }}
          </text>
        </view>
        <view class="goods-name">
          <view>
            ...
            <image
              class="gradeImg"
              v-if="state.isFlorist && state.serviceStaff && state.serviceStaff.gradeImg"
              mode="heightFix"
              :src="state.serviceStaff.gradeImg"
            ></image>
          </view>
        </view>
        <view
          class="desc-share"
          v-if="(state.isTeam && state.groupDescription) || state.description"
        >
          <view class="goods-desc">
            {{ state.isTeam ? state.groupDescription : state.description }}
          </view>
        ...
        <view
          ...
          v-if="
            state.pdtDetail.shipperType && state.pdtDetail.shipperType != 'STORE' && !state.zhiti
          "
        >
          {{ state.shipperTypeObj[state.pdtDetail.shipperType] }}
        </view>
        <view
          class="share-reamin"
          @click="handleClickShare"
          :style="
            (state.isSecKill && state.secKillInfo) ||
            state.goodType === 'advanceSell' ||
            state.isTeam
              ? 'top: 24rpx'
              : ''
        ...
        <view class="sold_box" v-if="!state.isService && !state.isFlorist">
          ...
          <view
            class="goods-soldNum"
            v-if="state.goodsList && state.goodsList[0] && state.goodsList[0].style != 'MEALS2'"
          >
            <text>库存</text>
            {{ state.balance }}
          </view>
          <view class="goods-soldNum" v-if="!state.isSecKill && !state.isTeam">
            <text>已售</text>
            {{ state.soldCount ? state.soldCount : 0 }}
          </view>
          <view class="desc-soldNum" v-if="state.isTeam || state.isSecKill">
            <text class="goods-soldNum">
              <text>已售</text>
              {{ state.soldCount ? state.soldCount : 0 }}
            </text>
            <text
        ...
        <view class="sold_box sold_box1" v-else>
          ...
          <view class="goods-soldNum goods-soldNum1">
            ...
            <text>服务时长</text>
            {{ state.serviceDuration1 ? state.serviceDuration1 + '-' : ''
            }}{{ state.serviceDuration }}分钟
          </view>
        </view>
        <view
          class="goods-delivery-info"
          v-if="state.storeInfo && state.storeInfo.merchantDelivery && state.storeSelfDeliveryInfo"
        >
          <image :src="state.imagesPath.iconStoreSelfDelivery" mode="widthFix"></image>
          {{ state.storeSelfDeliveryInfo.content }}
        </view>
      ...
      <view
        class="labels-board"
        v-if="state.goodsLabels.length > 0 && state.currentPage !== 'welfarePlan'"
      >
        ...
        <view class="labels-content-board">
          <view class="labels-content" v-for="(item, index) in state.goodsLabels" :key="index">
            <view class="labels-tag" v-if="item.labelName && !item.imageUrl">
              {{ item.labelName }}
            </view>
      ...
      <view class="team-box" v-if="state.isTeam && state.teamBuyList.length > 0">
        ...
        <view v-for="(item, index) in state.teamBuyList" :key="index" v-show="index < 2">
          <!--<template is="teamItem" :data="item"></template>-->
        </view>
        <view class="no-data" v-if="state.teamBuyList.length == 0">
          暂无正在进行中的拼团，赶快开团吧~
        </view>
      </view>
      <!-- 预售 -->
      <view class="goods-presell" v-if="state.advanceSell">
        ...
        <view class="presell-box">
          ...
          <view class="presell-item">
            <text class="top" v-if="state.advanceSellInfo.shipmentType === 'SELF'">
              {{ state.advanceSellInfo.payType === 'PARTIAL' ? '3.' : '2.' }}提货/发货时间
            </text>
            <text class="top" v-else-if="state.advanceSellInfo.shipmentType === 'EXPRESS'">
              {{ state.advanceSellInfo.payType === 'PARTIAL' ? '3.' : '2.' }}发货时间
            </text>
            <text class="top" v-else>
              {{ state.advanceSellInfo.payType === 'PARTIAL' ? '3.' : '2.' }}提货/发货时间
            </text>
            <!-- <view class="presell-icon"></view> -->
            ...
            <text
              v-if="
                state.advanceSellInfo.payType === 'PARTIAL' && state.advanceSellTime.deliverTime
              "
            >
              预售活动结束{{ state.advanceSellTime.deliverTime }}天后
            </text>
            <text
              v-if="state.advanceSellInfo.payType === 'FULL' && state.advanceSellTime.deliverTime"
            >
              预售活动结束{{ state.advanceSellTime.deliverTime }}天后
            </text>
            <text class="bottom" v-if="state.advanceSellInfo.orderSelfTime">
              {{ state.advanceSellTime.orderSelfTime }}
            </text>
          </view>
        </view>
      </view>
      <!-- 关联券 -->
      <view
        class="goods-related-coupon"
        @click="showRelateCoupon"
        v-if="state.relatedCouponList && state.relatedCouponList.length > 0"
      >
        <text style="flex-shrink: 0; width: 70rpx">领券:</text>
        <view class="related-coupon-board">
          <view
            class="goods-mini-coupon"
            v-for="(item, index) in state.relatedCouponList"
            :key="index"
          >
            {{ item.name }}
          </view>
      ...
      <!-- 商品详情 -->
      <view class="goods-details" v-if="state.productProperties.length > 0">
        <view
          v-for="(item, index) in state.productProperties"
          :key="index"
          v-show="item.propertyName !== '规格'"
        >
          <text>{{ item.propertyName }}</text>
      ...
      <!-- 关联商品 -->
      <view class="goods-comment" v-if="state.relatedProductList.length > 0">
      ...
      <view class="goods-appointment" v-if="state.isService || state.isFlorist">
        <view class="appointment-title">
          <text>预约</text>
          <view @click="appointmentConfirm" :style="'background-color: ' + state.themeColor + ';'">
            确认预约
          </view>
        </view>
        <scroll-view
          scroll-x=""
          enable-flex=""
          class="appointment-scroll goods-appointment-list"
          v-if="state.serviceItemList && state.serviceItemList.length > 0"
        >
          <view
            v-for="(item, index) in state.serviceItemList"
            :data-idx="index"
            :data-item="item"
            @click="item.isOptional ? 'changeService' : ''"
            class="goods-appointment-item"
            :style="
              (item.isOptional ? '' : 'background-color: #F2F2F2;') +
              (index == state.activeServiceIdx
          ...
          >
            <view class="img-box">
              <image
                :src="
                  ...
                  item.serviceStaffAvatar ||
                  item.serviceStaffImageUrl ||
                  item.imageUrl
                "
                mode="widthFix"
              ></image>
              <image
                class="service-staff-grade"
                :src="item.serviceStaffGradeImg"
                mode="heightFix"
              ></image>
            </view>
            <view>
            ...
            <!-- wx:if="{{isFlorist}}"  -->
            <view class="goods-earliest-time">最早可约{{ item.earliestTime }}</view>
            <view>
              <text class="goods-nowPrice">
                <text>￥</text>
                {{ filtToFix(item.activityPrice) }}
              </text>
              <text class="goods-oldPrice">
                <text>￥</text>
                {{ filtToFix(item.originalSellPrice) }}
              </text>
            </view>
            <view
              class="goods-check"
              :style="
                'background: ' +
                (index == state.activeServiceIdx ? state.themeColor : '#fff') +
                ';border: 1px solid ' +
                (index == state.activeServiceIdx ? state.themeColor : '#999') +
                ';'
              "
            >
              <image
                v-if="index == state.activeServiceIdx"
                style="width: 11px"
                :src="trueIcon"
                mode="widthFix"
              ></image>
        ...
        <scroll-view
          ...
          scroll-x=""
          enable-flex=""
          class="appointment-scroll week-list"
        >
          <view
            v-for="(item, index) in state.serviceItemList[activeServiceIdx].datetimeList"
            :data-item="item"
            :data-idx="index"
            @click="item.rest ? '' : 'changeWeek'"
            :class="'week-item ' + (item.rest ? 'week-disabled' : '')"
            :style="'color: ' + (index == state.activeWeekIdx ? state.themeColor : '#999999')"
          >
            {{ state.weekList[item.weekIndex] }}
            <view
              v-if="index == state.activeWeekIdx"
        ...
        <view v-if="state.serviceItemList && state.serviceItemList.length > 0" class="time-list">
          <view
            ...
            :data-item="item"
            :data-idx="index"
            @click="!item.isOptional || item.isDisabled ? '' : 'changeTime'"
            :class="'time-item ' + (!item.isOptional || item.isDisabled ? 'time-disabled' : '')"
            :style="
              (!item.isOptional ? 'border: 1px solid #DADADA;' : '') +
              'background-color: ' +
              (item.isSelect && item.isOptional
                ? state.themeColor
                : !item.isOptional || item.isDisabled
                  ? '#fff'
                  : '#F2F2F2') +
              ';color: ' +
              (item.isSelect ? '#fff' : '#333333')
            "
          >
            {{ item.time }}
        ...
        <view
          class="no-data-img-box"
          v-if="
            (!state.serviceItemList || state.serviceItemList.length == 0) && !state.serviceLoading
          "
        >
          <image
            class="no-data-img"
            :src="
              state.isFlorist ? state.imagesPath.noServiceItems : state.imagesPath.noServiceStaff
            "
            mode="widthFix"
          ></image>
        </view>
      ...
      <!-- 图文详情 -->
      <view class="goods-imgtext" id="productDetails" v-if="state.productDetails.length > 0">
      ...
      <!-- 商品评论 -->
      <view class="goods-comment" v-if="state.commentList.length > 0">
      ...
      <view
        :class="
          !state.productPlacedAtTheTop && state.isIphoneX
            ? 'nav-ios'
            : !state.productPlacedAtTheTop
              ? 'nav-android'
              : 'nav-height'
        "
      ></view>
      <!-- 底部按钮 -->
      <view
        :class="state.isIphoneX ? 'handle-box handle-box-ios' : 'handle-box'"
        v-if="!state.isService && !state.isFlorist"
      >
        ...
        <view @click="toShopCart" class="shopcart-icon">
          ...
          <text>购物车</text>
          <block v-if="state.shopCartNum > 0">
            <view class="badge">{{ state.shopCartNum }}</view>
          </block>
        </view>
        <view
          v-if="state.customerService != 'NONE'"
        ...
        <view
          v-if="
            !state.isTeam && !state.isSecKill && !state.advanceSell && state.goodType !== 'score'
          "
          :style="
            state.balance <= 0 || state.soldOut === true
              ? ''
              : state.themeColor
                ? 'border: 1px solid ' + state.themeColor
                : ''
          "
          :class="'normal-button ' + (state.balance <= 0 ? 'no-goods' : '')"
        >
          <block v-if="state.soldOut === true">
            <button class="soldOut-good">已下架</button>
          </block>
          <block v-else>
            <block v-if="state.balance > 0">
              <button
                v-if="!state.hasUserInfo"
                :style="'background-color: ' + state.themeColor"
                @click="getUserInfo"
              >
                {{ state.balance > 0 ? '加入购物车' : '已售罄' }}
              </button>
              <block v-else>
                <form
                  @submit="openGoodsSpecs"
                  v-if="state.sellPrice > 0"
                  @click.stop="addShopCartCatch"
                  class="f1"
                >
                  <button
                    class="add-cart"
                ...
                <form
                  @submit="openGoodsSpecs"
                  :style="state.sellPrice <= 0 ? 'width: 100%;' : ''"
                  @click.stop="addShopCartCatch"
                >
                  <button
                    form-type="submit"
                    :style="
                      (state.sellPrice <= 0 ? 'width: 100%;' : '') +
                      'background-color: ' +
                      state.themeColor
                    "
                    :data-id="state.goodsId"
            ...
            <block v-else>
              <button
                ...
                v-if="!state.hasUserInfo"
                :style="
                  'background-color: ' + state.themeColor + ';border: 1px solid ' + state.themeColor
                "
                @click="getUserInfo"
              >
                已售罄
        ...
        <view
          v-if="state.goodType === 'score'"
          :class="state.balance <= 0 || state.scoreInfo.score > state.score ? 'no-goods' : ''"
        >
          <form @submit="scoreExchange">
            <button
              v-if="!state.hasUserInfo"
              class="bg-009f55 btnw now-change"
              :style="'background-color: ' + state.themeColor"
              @click="getUserInfo"
            >
              立即兑换
            ...
            <view v-else>
              <button
                form-type="submit"
                class="btnw now-change"
                :data-id="state.goodsId"
                :style="'background-color: ' + state.themeColor"
                v-if="state.balance > 0 && state.score >= state.scoreInfo.score"
              >
                立即兑换
              </button>
              <button
        ...
        <view
          v-if="state.isTeam"
          :style="state.balance <= 0 ? '' : 'border: 1px solid ' + state.themeColor"
          :class="'team-button ' + (state.balance <= 0 ? 'no-goods' : '') + ' f1'"
        >
          ...
          <view v-else>
            ...
            <view v-if="state.teamBuyingType === 'COMMON'">
              <button
                class="group"
                :style="'background-color: ' + state.themeColor"
                v-if="state.groupBalance > 0"
                data-type="leader"
                @click="openTeamBuySpecs"
              >
                <text>￥{{ filtToFix(state.teamAllPrice) }}</text>
                <text>立即开团</text>
              </button>
              <button
                class="group"
            ...
            <view v-if="state.teamBuyingType === 'THOUSAND'">
              <form @submit="joinThousandGroup" @click.stop="addShopCartCatch">
                <button
                  ...
                  v-if="state.groupBalance > 0"
                >
                  <text>￥{{ filtToFix(state.teamMemberAllPrice) }}</text>
                  <text>立即参团</text>
                </button>
                <button
                  form-type="submit"
        ...
        <!-- 秒杀 -->
        <view
          v-if="state.isSecKill"
          :style="'border: 1px solid ' + state.themeColor + ';'"
          :class="
            'seckill ' +
            (state.secKillInfo.availableStockAmount == 0 || state.balance <= 0
              ? 'no-seckill-goods'
              : '')
          "
        >
          ...
          <view v-else>
            <view v-if="state.secKillInfo.secKillStatus === 'created'">
              ...
              ></button>
              <button
                style="background-color: #d9d9d9; width: 460rpx"
                data-type="grab"
                @click="openGrabGoodsSpecs"
              >
            ...
            <view v-else>
              ...
              <button
                v-if="state.secKillInfo.availableStockAmount > 0 && state.balance > 0"
                :style="'background-color: ' + state.themeColor"
                data-type="grab"
                @click="openGrabGoodsSpecs"
              >
        ...
        <!-- 预售 -->
        <view
          v-if="state.advanceSell"
          :class="
            'advanceSell_button ' +
            (state.advanceSellInfo.advanceAmount == 0 || !state.advanceSellInfo.balance
              ? 'no-goods'
              : '')
          "
        >
          ...
          <button
            class="btnw"
            v-if="!state.hasUserInfo"
            style="background-color: #7e21ec"
            @click="getUserInfo"
          >
            {{
              state.isAdvanceSellActiveEnd
                ? '活动已结束'
                : state.isAdvanceSellActiveStart
                  ? '立即预定'
                  : '活动未开始'
            }}
          </button>
          <button
            class="btnw"
            v-else-if="
              state.advanceSellInfo.shelvesStatus == 'OFF' ||
              state.advanceSellInfo.shelvesStatus == 'DELETED'
            "
            style="background-color: #7e21ec"
            @click="toPayAdvanceSell"
          >
            已下架
          </button>
          <view v-else>
            <button
              class="btnw"
              v-if="
                state.advanceSellInfo.advanceAmount > 0 &&
                state.advanceSellInfo.balance &&
                !state.isAdvanceSellActiveEnd
              "
              style="background-color: #7e21ec"
              @click="handleNext"
            >
              {{ state.isAdvanceSellActiveStart ? '立即预定' : '活动未开始' }}
            </button>
            <button
              class="btnw"
              v-else-if="
      ...
      <view class="modal-box" v-if="state.groupModal" data-type="group" @click="clickBlank">
        <view class="modal-body" @click.stop="clickBody">
          <view class="modal-content join-group">
            ...
            <view class="group-info">
              <view>
                仅剩余
                <text>{{ state.groupInfo.teamMemberCount - state.groupInfo.joinMemberCount }}</text>
                个参团名额可用,赶紧来参团吧!
              </view>
              <view>
                ...
                火热拼团ing
                <text>
                  {{ state.countDownTime.hou }}:{{ state.countDownTime.min }}:{{
                    state.countDownTime.sec
                  }}
                </text>
                后结束。
              </view>
            ...
            <view class="group-people-list">
              <view v-if="state.groupInfo.teamBuyingType !== 'THOUSAND'">
                <view
                  ...
                  v-for="(item, idx) in state.groupInfo.joinMbrs"
                  :key="idx"
                  v-show="idx == 0 && state.groupInfo.joinPeopleCount > 0"
                >
              ...
              <view v-if="state.groupInfo.teamBuyingType === 'THOUSAND'">
                <view
                  class="people-item"
                  v-for="(item, index) in state.groupInfo.joinMbrs"
                  :key="index"
                  v-show="
                    state.groupInfo.teamPeopleCount > 10
                      ? index < 9
                      : index < state.groupInfo.teamPeopleCount
                  "
                >
                  <image :src="item.avatar"></image>
                </view>
                <view
                  class="people-item"
                  v-for="(item, index) in state.groupInfo.teamPeopleCount -
                  state.groupInfo.joinPeopleCount"
                  v-show="
                    state.groupInfo.teamPeopleCount > 10
                      ? index < 9 - state.groupInfo.joinPeopleCount
                      : state.groupInfo.teamPeopleCount - state.groupInfo.joinPeopleCount
                  "
                  :key="index"
                >
                  <text>?</text>
              ...
              <view v-if="state.groupInfo.teamBuyingType !== 'THOUSAND'">
                <view
                  ...
                  :key="index"
                  v-show="
                    index > 0 && state.groupInfo.teamPeopleCount > 10
                      ? index < 8
                      : index < state.groupInfo.teamPeopleCount
                  "
                >
            ...
            <button v-if="!state.hasUserInfo" class="button-box" @click="getUserInfo">
              确认拼团
            </button>
            <button v-else-if="!state.isMember" class="button-box" @click="toAuthorize">
              确认拼团
            </button>
            <button v-else class="button-box" data-type="member" @click="openTeamBuySpecs">
              确认拼团
            </button>
          </view>
        </view>
      </view>
      <view class="modal-box" v-if="state.modal" data-type="goods" @click="clickBlank">
        <view class="modal-body" @click.stop="clickBody">
          <view class="modal-content handle-goods">
            ...
            <view class="goods-box">
              ...
              <view class="goods-info">
                ...
                <view class="goods-price-box" v-if="state.isTeam">
                  <text>开团价：￥{{ filtToFix(state.teamLeaderPrice) }}</text>
              ...
              <view class="count-box">
                ...
                <view
                  ...
                  :style="
                    ...
                    state.themeColor +
                    ';border: 1px solid #fb5606;border-color: ' +
                    state.themeColor +
                    ';'
                  "
                >
            ...
            <view class="button-box">
              <button
                class="confirm"
                :style="'background: ' + state.themeColor + ';border-color:' + state.themeColor"
                :data-id="goodsInfo.id"
                v-if="state.advanceSell"
                @click.stop="toPayAdvanceSell"
              >
                下一步
              </button>
              <button
    ...
    <!-- 普通商品添加购物车 -->
    <popup
      v-if="state.show.goodsNormal"
      :show="state.show.goodsNormal"
      position="bottom"
      custom-class="bottom"
      ...
      <view class="spec-box">
        <view class="spec-top-box">
          ...
          <view class="price-box">
            <view class="price" v-if="state.isTeam && state.goodtype != 'teambuy'">
              拼团价￥
              <text>{{ filtToFix(state.teamAllPrice) }}</text>
            </view>
            <view class="price" v-else-if="state.isSecKill">
              秒杀价￥
              <text>{{ filtToFix(state.secKillInfo.price) }}</text>
            </view>
            <view class="price" v-else-if="state.isBargainPrice">
              最低可砍至￥
              <text>{{ filtToFix(state.sellPrice) }}</text>
            </view>
            <view class="price" v-else-if="state.isSolitairePrice">
              接龙价￥
              <text>{{ filtToFix(state.sellPrice) }}</text>
            </view>
            <view
              ...
              v-else-if="
                !state.isTeam &&
                state.promotionPrice &&
                (!state.sellPrice || state.promotionPrice < state.sellPrice) &&
                (!state.memberPrice || state.promotionPrice < state.memberPrice)
              "
            >
              促销价￥
              <text>
            ...
            <view
              ...
              v-else-if="
                state.memberPrice &&
                state.memberPrice < state.sellPrice &&
                state.memberPrice < state.originalPrice
              "
            >
              {{ state.vipGradeConfig.priceLabel }}￥
              <text>
                {{
                  state.specificationsFlag
                    ? filtToFix(state.promotionMinSellPrice) +
        ...
        <scroll-view
          class="spec-con-box"
          scroll-y="true"
          style="min-height: 310rpx; max-height: 580rpx; height: 175rpx"
        ></scroll-view>
        ...
        <view class="spec-button-box">
          <view class="spec-button-box-content">
            <form @submit="confirm" @click.stop="noop">
              <button
                v-if="state.activityType === 'normal'"
                form-type="submit"
                class="spec-button"
    ...
    >
      <goods-spec
        @sendSpecShareImg="sendSpecShareImg"
        :specs="state.specs"
        :activeSpecId="state.shopCarSpecId"
        :specDetails="state.specDetails"
        :memberPrice="state.memberPrice"
        ...
        :has-immediately-grab="state.hasImmediatelyGrab"
        @confirm="confirmSpecs"
        :minSellPrice="state.minSellPrice"
        :maxSellPrice="state.maxSellPrice"
      ></goods-spec>
    </popup>
    <!-- 关联券弹窗 -->
    <popup
      v-if="state.show.relateCoupon"
      :show="state.show.relateCoupon"
      position="bottom"
      custom-class="bottom"
      @close="toggleRelateCouponPopup"
    >
      <view class="popup-relate-coupon-board">
        ...
        <scroll-view
          :scroll-y="true"
          class="popup-content"
          :style="state.relatedCouponList.length > 1 ? 'height:716rpx;' : ''"
        >
          <view v-for="(item, index) in state.relatedCouponList" :key="index">
            <coupon-receive
              :coupon="item"
              :hasUserInfo="state.hasUserInfo"
    ...
    <!-- 拼团规则弹窗 -->
    <popup
      v-if="state.show.rulesDetail"
      :show="state.show.rulesDetail"
      position="middle"
      class="rules"
      custom-class="middle"
      @close="toggleRulesDetailPopup"
    >
      <view class="popup-rules-detail-board">
        ...
        <image
          ...
          @click="toggleRulesDetailPopup"
        ></image>
        <view class="popup-content">
          <view>
            <text class="rule-title">1.开团：</text>
            在拼团活动期间，用户进入拼团商品详情页，点击立即开团，下单完成支付后视为开团成功，并成为该团团长。
          </view>
          <view>
            <text class="rule-title">2.参团：</text>
            进入他人团详情页面，点击参团，下单完成支付后视为开团成功。
          </view>
          <view>
            <text class="rule-title">3.优惠券和积分的使用：</text>
            除特定优惠券外，购买拼团商品时，一般不可使用优惠券。
          </view>
          <view>
            <text class="rule-title">4.拼团成功：</text>
            在成团有效期内（若距离拼团活动结束时间小于成团有效期，则以活动结束时间为准），完成支付的用户达到达成指定拼团人数，则拼团成功。
          </view>
          <view>
            <text class="rule-title">5.拼团失败：</text>
            在拼团有效期后，完成支付的用户未达到指定拼团人数，则拼团失败。（若高峰期间同时支付的人过多，将以支付时间先后为准，超出拼团人数限制的用户将自动新开一个团成为团长，需要另外再邀请/等待其他用户参团）
          </view>
          <view>
            <text class="rule-title">6.订单退款退货：</text>
            拼团订单在发货前支持退款，在配送中不支持发起退款。用户在收到拼团商品后，若不满拼团商品质量，双方经协商可由商家在后台发起退货退款流程。
          </view>
        </view>
      </view>
    </popup>
    ...
    <share
      v-if="state.show.showSharePop"
      :show="state.showSharePop"
      :specificationsFlag="state.specificationsFlag"
    ...
    <view
      class="good_box"
      v-if="!state.hide_good_box && state.goodsBoxImage"
    ...
    <timeout
      v-if="state.showTimeout"
      :show="state.showTimeout"
      @close="timeoutPopupClose"
    ></timeout>
  </view>
</template>
<script setup>
...
import righticon from '@/utils/newretail/image/righticon.png'
...
const state = reactive({
  ...
  promotionMinSellPrice: 0,
  promotionMaxSellPrice: 0,
...
function getServiceConfig() {
  sysService
    .query('serviceDuration,serviceTimeInterval')
    .then((res) => {
      if (res && res.length > 0) {
...
function appointmentConfirm() {
  ...
  try {
    ...
    if (state.options && state.options.room_id) {
      shopcart = {
        ...shopcart,
        orderSource: 'WX_MINI_APP-LIVE',
        room_id: state.options.room_id,
      }
    }
    if (state.isFlorist) {
  ...
  function toPerfectOrder() {
    let easyScene = 'none'
    if (state.options.easyScene != null) {
      easyScene = state.options.easyScene
    }
...
function getWeekAndTime(currentTime, currentUser) {
  ...
  if (state.isService || state.isFlorist) {
    state.shareData.sellPrice = state.activeService
      ? state.activeService.activityPrice
      : state.serviceMinPrice
  ...
  state.serviceTimeList.forEach((item, serviceIdx) => {
    ...
    if (item.datetimeList && item.datetimeList.length > 1) {
      ...
      item.datetimeList.forEach((date, dateIdx) => {
        ...
        if (
          state.activeServiceIdx == serviceIdx &&
          !date.rest &&
          !state.activeWeekIdx &&
          state.activeWeekIdx !== 0
        ) {
        ...
        date.time.forEach((time, idx) => {
          ...
          if (
            date.rest ||
            (date.scheduledList &&
              (date.scheduledList.indexOf(time) > -1 ||
                (currentTime == time &&
                  currentTime &&
                  date.scheduledList.indexOf(currentTime) > -1)))
          ) {
          ...
          if (
            date.rest ||
            (date.manualList &&
              (date.manualList.indexOf(time) > -1 ||
                (currentTime == time && currentTime && date.manualList.indexOf(currentTime) > -1)))
          ) {
            timeItem.isOptional = false
          }
          ...
          if (
            currentTime == time &&
            currentTime &&
            ((date.manualList && date.manualList.indexOf(currentTime) > -1) ||
              (date.scheduledList && date.scheduledList.indexOf(currentTime) > -1) ||
              (date.noTimeList && date.noTimeList.indexOf(time) > -1)) &&
            dateIdx === state.activeWeekIdx
          ) {
          ...
          // }
          if (
            serviceIdx == state.activeServiceIdx &&
            dateIdx == state.activeWeekIdx &&
            (state.activeTimeIdx || state.activeTimeIdx === 0) &&
            state.activeTimeIdx == idx
          ) {
            timeItem.isSelect = true
          }
  ...
  if (
    !state.activeWeekIdx &&
    state.serviceTimeList[state.activeServiceIdx].datetimeList &&
    state.serviceTimeList[state.activeServiceIdx].datetimeList[state.activeWeekIdx]
  ) {
...
function changeTime(e) {
  ...
  let flag =
    activeIdx >
    state.serviceItemList[state.activeServiceIdx].datetimeList[state.activeWeekIdx].newTimeList
      .length -
      selectLength
  if (!flag) {
    flag = state.serviceItemList[state.activeServiceIdx].datetimeList[
      state.activeWeekIdx
    ].newTimeList.some((item, idx) => {
      return (
        (!item.isOptional || item.isDisabled) &&
        (activeIdx || activeIdx === 0) &&
        activeIdx <= idx &&
        idx < activeIdx + selectLength
      )
    })
  }
  if (flag) {
  ...
  getWeekAndTime(
    state.activeTimeIdx || (state.activeTimeIdx === 0 && activeItem) ? activeItem.time : null,
  )
}
...
function getGoodsDetails(storeId, productId, type) {
  ...
  if (productId) {
    ...
    if (type === 'normal') {
      ...
      console.log('----------抢购商品----------')
      getSecKillGoodsDetails(state.options.activityId, productId, storeId)
      getProductsComment(productId)
      getPackagingInfo(productId)
    } else if (type === 'advanceSell') {
      state.goodType = 'advanceSell'
      console.log('----------预售商品----------')
      getAdvanceSellDetails(state.options.advanceId, productId, storeId)
      getProductsComment(productId)
    ...
    if (
      state.options.shareId &&
      state.options.shareId != '' &&
      state.options.shareId != undefined
    ) {
      const shareId = state.options.shareId
      try {
...
async function setSharePeopleInfo(productId) {
  if (state.options.shareId && state.options.shareId != '' && state.options.shareId != null) {
    const shareId = state.options.shareId
...
onLoad(function (options) {
  ...
  if (state.options.scene) {
    ...
    if (scene.indexOf('gmparam:') >= 0) {
      ...
      state.options.productId = sceneArr[0]
      state.options.productId = sceneArr[0]
      state.options.type = sceneArr[1]
      state.options.type = sceneArr[1]
      if (state.options.type === 'advanceSell' && sceneArr[2] != null) {
        state.options.advanceId = sceneArr[2]
      }
      // 分享信息打开统计 扫码默认为分享
      ANALYSIS.ShareOpenAnalysis()
    }
  } else if (state.options.q) {
    // state.options 中的 q 需要使用 decodeURIComponent 才能获取到生成二维码时传入的 scene
    const scene = decodeURIComponent(state.options.q)
    const sceneArr = scene.split(',')
    state.options.prodCode = sceneArr[0]
    ...
    arr.forEach((item) => {
      ...
      if (item.slice(0, num) == 'prodCode') {
        state.options.prodCode = item.slice(num + 1)
      } else if (item.slice(0, num) == 'sourceOrderNo') {
        state.options.sourceOrderNo = item.slice(num + 1)
      } else if (item.slice(0, num) == 'storeCode') {
        state.options.storeCode = item.slice(num + 1)
        // that.setData({
        //   storeId: item.slice(num + 1)
        // })
      } else if (item.slice(0, num) == 'posNo') {
        state.options.posNo = item.slice(num + 1)
      }
    })
  }
  ...
  // 组合商品点击购物车
  if (state.options.openFlag && state.options.openFlag === 'open') {
    state.meals.productId = state.options.productId
    state.meals.show = true
    state.meals.storeId = state.options.storeId
  ...
  if (state.options.storeId) {
    if (!options) {
      state.options = {
        storeId: state.options.storeId,
      }
    } else {
      state.options.storeId = options.storeId
    }
    state.storeId = state.options.storeId
    getRecentlyStore(state.options)
  }
  // 判断globalData.storeInfo有没有数据，有数据，则使用选择的门店数据，没有，则根据当前地理位置获取最近门店
  // state.options.storeId == app.globalData.storeInfo.id &&
  // || (state.options.storeId != app.globalData.storeInfo.virtualStoreId)
  if (app.globalData.storeInfo && !state.options.storeId) {
    ...
    //   } else if (state.options.productId != null && state.options.productId !== "") {
    //     self.getGoodsDetails(app.globalData.storeInfo.id, state.options.productId, state.options.type)
    //   }
    if (state.options.zhiti === 'zhiti') {
      console.log('进入自提')
      ...
      if (sceneId) {
        ...
        getGoodsDetails(storeIdactive.id, state.options.productId, state.options.type)
      } else if (state.options.prodCode != null && state.options.prodCode !== '') {
        productService
          .getByExternalCodeStore(state.options.storeCode || state.storeId)
          .then((res) => {
            productService.getByExternalCodeMall(state.options.prodCode).then((resl) => {
              getGoodsDetails(res.id, resl.id, state.options.type)
      ...
      getStoreInfoById(
        state.options.storeId ||
          app.globalData.storeInfo.virtualStoreId ||
      ...
      if (sceneId) {
        ...
        productService.getByExternalCodeStore(state.options.storeCode).then((res) => {
          productService.getByExternalCodeMall(state.options.prodCode).then((resl) => {
            getGoodsDetails(res.id, resl.id, state.options.type)
            app.globalData.storeInfo = res || app.globalData.storeInfo
            state.productId = resl.id
    ...
    if (state.options.storeId) {
      if (app.globalData.storeInfo) {
        app.globalData.storeInfo.id = state.options.storeId
      } else {
        app.globalData.storeInfo = {
          id: state.options.storeId,
        }
      }
    }
    checkLocationAuth(state.options)
...
onShareAppMessage(function () {
  ...
  // this.drawShareImg();
  let sellPrice =
    state.promotionPrice && state.promotionPrice < state.sellPrice
      ? state.promotionPrice
      : state.sellPrice
  ...
  if (state.goodType === 'advanceSell') {
    title =
      '【预售】' +
      state.pdtDetail.sellPrice +
      '元' +
      '/' +
      state.pdtDetail.frontPrice +
      '元定金' +
      state.name
  }
  ...
  if (app.globalData.userInfo && app.globalData.userInfo.member) {
    ...
    path =
      path +
      '&storeId=' +
      (state.storeInfo && state.business != 'DISTRIBUTION' && state.business != 'NEXT_DAY'
...
function getShareImgInfo() {
  ...
  if (state.productPictures[0].fileType !== 'VIDEO') {
    firstPic = state.productPictures[0].url
      ? state.productPictures[0].url.replace('http://', 'https://')
      : ''
                ...
                success(res) {
                  if (res) {
                    ...
                    function activityDesc(num) {
                      ...
                      ctx.fillText(
                        state.secKillInfo.secKillStatus === 'created' ? '距开始' : '距结束',
                        230,
                        200,
                      )
                      ctx.draw(true)
            ...
            .then((res) => {
              const opts = {
                products: [
                  {
                    productId,
                    traceId: state.options.traceId != null ? state.options.traceId : '',
                  },
                ],
              }
      ...
      shopcartService
        ...
        .then((res) => {
          ...
          for (let i = 0; i < res.items.length; i++) {
            ...
            if (item.productId === state.options.productId && item.grabActivityId != null) {
          ...
          if (res.items && res.items.length > 0) {
            ...
            res.items.forEach((item) => {
              if (
                item.productId == state.goodsList[0].id &&
                item.grabActivityId == state.secKillInfo.activityId
              ) {
    ...
    if (canAddToCart === false) {
      ...
      if (style !== 'SPEC_PARENT' && style !== 'MEALS2') {
        ...
        let opts =
          '?productId=' +
          productId +
          (business === 'DISTRIBUTION' || business === 'NEXT_DAY'
            ? storeId
              ? '&storeId=' + storeId
              : ''
            : '&storeId=' + state.storeId) +
...
function queryShopcart() {
  if (state.options.type === 'secondkill' || state.isSecKill) {
    shopcartService
      .getCatering(state.storeId)
      .then((res) => {
        console.log(res)
        ...
        for (let i = 0; i < res.items.length; i++) {
          const item = res.items[i]
          if (
            (item.productId === state.options.productId || item.productId === state.goodsId) &&
...
function handleReduce(e) {
  ...
  if (
    num > 1 &&
    (!state.secKillInfo.minQty || state.secKillInfo.minQty < num) &&
    (!state.minQty || state.minQty < num)
  ) {
    num--
  ...
  if (loginStatus) {
    ...
    goodsList.forEach((item) => {
      item = {
        ...
        productId: item.id,
        productNum:
          state.productNum < state.teamMemberJoinCount
            ? state.teamMemberJoinCount
            : state.productNum
              ? state.productNum
              : 1,
        initialPurchaseNumber: state.initialPurchaseNumber || 1,
      ...
      if (state.groupBalance > 0) {
        try {
          ...
          let teamBuy = {
            ...
            storeId: state.storeId,
            goodsList: [],
            teamBuyId: state.options.productId,
            orderType: 'group',
      ...
      if (loginStatus && state.isMember) {
        shopcartService
          ...
          .then((res) => {
            ...
            if (state.isCateringMeals) {
              return getCateringDetailsById(state.options.productId)
            }
          })
          .then((res) => {
            // 餐饮套餐商品
            if (state.balance > 0) {
              if (state.secKillInfo.availableStockAmount > 0) {
                if (
                  state.secKillInfo.minQty &&
                  state.secKillInfo.minQty > state.secKillInfo.availableStockAmount
                ) {
                  uni.showToast({
                    title: '库存数量小于起购数量~',
                    icon: 'none',
                ...
                try {
                  ...
                  productsList.forEach((item) => {
                    item = {
                      ...
                      productId: item.id,
                      productNum:
                        state.secKillInfo.minQty && state.secKillInfo.minQty > state.productNum
                          ? state.secKillInfo.minQty
                          : state.productNum,
                      initialPurchaseNumber: state.initialPurchaseNumber || 1,
    ...
    .then((res) => {
      if (res) {
        ...
        if (res.teamMemberJoinCount != null) {
          ...
          state.teamMemberAllPrice = parseFloat(
            (res.teamMemberPrice * res.teamMemberJoinCount).toFixed(2),
          )
        } else {
        ...
        function countDown() {
          ...
          if (state.groupStatus !== 'FINISHED') {
            ...
            if (
              state.groupTime.hou == '00' &&
              state.groupTime.min == '00' &&
              state.groupTime.sec == '00'
            ) {
...
function handleScoreProductDetails(storeId, productId, response) {
  ...
  tempData.push(res)
  let balance =
    Number(response.total) > Number(res.balance) ? Number(res.balance) : Number(response.total)
    ...
    for (let i = 0; i < res.labels.length; i++) {
      ...
      if (ele.type === 'PROMOTIONLABEL') {
        ...
        for (let i = 0; i < tempLabels.length; i++) {
          ...
          if (
            (it.labelName && it.labelName === ele.labelName) ||
            (it.imageUrl && it.imageUrl === ele.imageUrl)
          ) {
            isHas = true
          }
        }
      ...
      .then((res) => {
        ...
        if (res) {
          ...
          if (
            ...
            const balance =
              res.balance > teamBuyings.productBalance ? teamBuyings.productBalance : res.balance
          ...
          if (res.business === 'SCORE') {
            ...
            (res.business === 'CATERING' ||
              res.business === 'DISTRIBUTION' ||
              res.business === 'NEXT_DAY') &&
            res.style === 'MEALS' &&
            (state.goodType == 'normal' ||
              state.goodType == 'group' ||
              state.goodType == 'secondkill')
          ) {
            // 标识餐饮套餐商品
          ...
          if (res.style === 'SPEC_PARENT') {
            ...
            if (state.isSecKill) {
              ...
              getActivityGoodsSpec(state.options.productId, productId, storeId, 'group')
            } else {
              getGoodsSpec(productId, storeId)
            }
          } else if (res.style === 'MEALS2') {
          ...
          // 判断秒杀
          if (
            state.options.type === 'secondkill' &&
            state.options.goodsList &&
            state.options.goodsList != null
          ) {
            const secKillData = JSON.parse(state.options.goodsList)
          ...
          // 判断拼团
          if (state.options.type === 'group') {
            state.sellPrice =
              res.sellPrice >= 0 ? (res.sellPrice * state.productNum).toFixed(2) : ''
            state.originalPrice = res.originalPrice
              ? (res.originalPrice * state.productNum).toFixed(2)
              : ''
          } else {
            state.sellPrice = res.sellPrice ? res.sellPrice : ''
            state.originalPrice = res.originalPrice ? res.originalPrice : ''
          ...
          if (teamBuyings != null) {
            ...
            if (teamBuyings.sharePicture == null || teamBuyings.sharePicture == '') {
              ...
              util
                .createGroupSharePictures(state.productPictures[0].url, group)
                .then((picture) => {
          ...
          if (item.datetimeList && item.datetimeList.length > 1) {
            ...
            item.datetimeList.forEach((date, dateIdx) => {
              date.time.forEach((time) => {
                ...
                if (
                  (date.scheduledList && date.scheduledList.indexOf(time) > -1) ||
                  (date.manualList && date.manualList.indexOf(time) > -1) ||
                  date.rest
                ) {
                  timeItem.isOptional = false
                }
                const appointmentTimeList =
                  appointmentTime && appointmentTime.split(' ').length > 0
                ...
                if (
                  ...
                  (!appointmentTime ||
                    new Date(time).getTime() <
                      new Date(appointmentTimeList[appointmentTimeList.length - 1]).getTime())
                ) {
                  const dateList = date.day ? date.day.split('-') : []
                  appointmentTime =
                    (dayText != date.day ? dateList[dateList.length - 1] + '日' : ' ') + time
                }
                const earliestTimeList =
                  earliestTime && earliestTime.split(' ').length > 0 ? earliestTime.split(' ') : []
                if (
                  timeItem.isOptional &&
                  !timeItem.isDisabled &&
                  (!earliestTime ||
                    new Date(time).getTime() <
                      new Date(earliestTimeList[earliestTimeList.length - 1]).getTime())
                ) {
                  const dateList = date.day ? date.day.split('-') : []
                  earliestTime =
                    (dayText != date.day ? dateList[dateList.length - 1] + '日' : ' ') + time
        ...
        if (res) {
          ...
          if (serviceList && serviceList.length > 0) {
            ...
            serviceList.forEach((item) => {
              ...
              if (
                (item.originalSellPrice && item.originalSellPrice > originalSellPrice) ||
                !originalSellPrice
              ) {
  ...
  advanceSellService.getByIds(advanceId, productId, storeId).then((res) => {
    ...
    if (tempData.payType === 'PARTIAL') {
      ...
      balanceDate =
        balanceStartTime +
        ' ' +
        timeDivision(tempData.balanceStartTime) +
        '\n' +
        '~' +
        balanceEndTime +
        ' ' +
        timeDivision(tempData.balanceEndTime)
    ...
    if (res.selfTimeScope) {
      ...
      orderSelfTime =
        selfStartTime +
        ' ' +
        timeDivision(res.selfTimeScope.split('~')[0]) +
        '\n' +
        '~' +
        selfEndTime +
        ' ' +
        timeDivision(res.selfTimeScope.split('~')[1])
    ...
    let advanceSellDate =
      frontStartTime +
      ' ' +
      timeDivision(tempData.frontStartTime) +
      '\n' +
      '~' +
      frontEndTime +
      ' ' +
      timeDivision(tempData.frontEndTime)
...
function toPayAdvanceSell() {
  ...
  if (
    ...
    if (loginStatus && state.productNum != '' && state.productNum != 0) {
      try {
        const goodsPrice =
          parseFloat(state.advanceSellInfo.frontPrice.toFixed(2)) * state.productNum
...
function sendSpecShareImg(shareImg) {
  ...
  if (shareImg.detail) {
    ...
    state.specOriginalPrice =
      shareImg.detail.originalPrice > 0 ? shareImg.detail.originalPrice : null
  }
}
...
async function handleShareImageData() {
  ...
  if (state.isService || state.isFlorist) {
    shareData.sellPrice = state.activeService
      ? state.activeService.activityPrice
      : state.serviceMinPrice
    ...
    .then((res) => {
      if (res && res.stratumRate) {
        ...
        if (state.promotionPrice) {
          ...
          if (
            state.memberPrice &&
            state.memberPrice < state.promotionPrice &&
            state.vipGradeConfig.openMemberPrice
          ) {
...
function getGoodsSpec(productId, storeId) {
  ...
  productService
    ...
    .then((res) => {
          ...
          function getPrice(priceItem) {
            if (Array.isArray(priceItem)) {
              ...
              if (priceItem.sellPrice < minSellPrice) {
                ...
                if (
                  priceItem.memberPrice &&
                  (!priceItem.promotionPrice ||
                    priceItem.promotionPrice >= priceItem.memberPrice) &&
                  priceItem.memberPrice >= maxSellPrice &&
                  priceItem.memberPrice <= priceItem.sellPrice
                ) {
                  maxSellPrice = priceItem.memberPrice
                } else if (
                  priceItem.promotionPrice &&
                  (!priceItem.memberPrice || priceItem.memberPrice >= priceItem.promotionPrice) &&
                  priceItem.promotionPrice >= maxSellPrice &&
                  priceItem.promotionPrice <= priceItem.sellPrice
                ) {
              ...
              if (
                ...
                (((!priceItem.promotionPrice ||
                  priceItem.promotionPrice >= priceItem.memberPrice) &&
                  priceItem.memberPrice < minSellPrice &&
                  priceItem.memberPrice <= priceItem.sellPrice) ||
                  minSellPrice === null)
              ) {
                ...
                priceItem.promotionPrice &&
                (((!priceItem.memberPrice || priceItem.memberPrice >= priceItem.promotionPrice) &&
                  priceItem.promotionPrice < minSellPrice &&
                  priceItem.promotionPrice <= priceItem.sellPrice) ||
                  minSellPrice === null)
              ) {
              ...
              if (maxSellPrice === null) {
                if (
                  priceItem.memberPrice &&
                  (!priceItem.promotionPrice || priceItem.promotionPrice < priceItem.memberPrice) &&
                  priceItem.memberPrice >= maxSellPrice &&
                  priceItem.memberPrice <= priceItem.sellPrice
                ) {
                  maxSellPrice = priceItem.memberPrice
              ...
              if (
                (priceItem.promotionPrice && priceItem.promotionPrice < promotionPrice) ||
                promotionPrice === null
              ) {
      ...
      if (state.options.source === 'addshopcart') {
        openGoodsSpecs()
...
function getActivityGoodsSpec(activityId, productId, storeId, type) {
  ...
  if (type === 'group') {
    ...
    grabChildrenProduct = []
    secKillService
      .getChildrenProduct(parentGrabProductId)
      .then((res) => {
...
function openGoodsSpecs(e) {
  ...
  if (state.specs.length > 0) {
    ...
    if (goodsStyle === 'normal') {
      ...
      state.meals.storeId =
        state.business == 'DISTRIBUTION'
          ? state.distributionStoreId
          : state.business == 'NEXT_DAY'
            ? state.nextDayStoreId
            : app.globalData.storeInfo.id
      // if(addType == 'detail') {
      ...
      if (state.balance > 0) {
        ...
        try {
          ...
          let shopcart = {
            ...
            goodsList: [],
            orderType:
              state.goodsList[0].business === 'DISTRIBUTION' && !state.zhiti
                ? 'distribution'
                : state.goodsList[0].business === 'NEXT_DAY'
                  ? 'nextDay'
                  : 'normal',
            scource: 'BUYNOW',
          }
...
function confirmSpecs(e) {
  ...
  state.initialPurchaseNumber = e.detail.initialPurchaseNumber
    ? parseInt(e.detail.initialPurchaseNumber)
    : 1
  ...
  if (state.activityType === 'group') {
    ...
    // 关闭弹窗
    toggleGoodsSpecPopup()
    state.options.productId = e.detail.productId
    ...
    if (addType && addType === 'buyNow') {
      ...
      state.goodsList[0].usePromotionPrice = !!(
        e.detail.promotionPrice &&
        e.detail.promotionPrice != '0' &&
        e.detail.promotionPrice < e.detail.sellPrice
      )
...
function callPhone() {
  ...
  NAVPAGE.toCustomService(
    `?goodsId=${state.goodsId}&picture=${picture}&title=${title}&sellPrice=${sellPrice}&description=${description}&goodPageData=${JSON.stringify(state.options)}`,
  )
}
  ...
  storeService
    ...
    .then((res) => {
      if (
        state.options &&
        state.options.storeId &&
        (!app.globalData.distributionStore || res.id != app.globalData.distributionStore.id) &&
        (!app.globalData.nextDayStore || res.id != app.globalData.nextDayStore.id)
      ) {
        app.globalData.storeInfo = {
          ...res,
      ...
      if (
        !res ||
        !res.merchantDelivery ||
        !res.storeSelfDeliveryTimes ||
        res.storeSelfDeliveryTimes.length === 0
      ) {
        return
      }
      console.log('有进入吗？')
...
function goodsDetailsBuryingPoint(productId, storeId) {
  const that = this
  const traceId = state.options.traceId != null ? state.options.traceId : ''
  const opts = {
    products: [
      {
        productId,
