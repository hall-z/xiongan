<template>
  <!-- pages/mallModule/index/index/index.wxml -->
  ...
  <view
    ...
    "
  >
    <view class="logo" @click="changeStore" :style="'color: ' + state.themeColor">
  ...
  <view v-if="state.category.length > 0">
    <view>
      ...
      ></category-vertical>
      <!-- <view class="setShop" bindtap='bindgoShop' >购物车</view> -->
      <view class="flow-car" @click="bindgoShop">
...
// pages/mallModule/tabbar/selfLifting/selfLifting.js
...
function handleConfirm() {
  uni.navigateTo({
    ...
    url:
      '/pages-sub/newretail/pages/mallModule/goods/search/search?linktypea=' +
      'self' +
      '&storeId=' +
      state.storeId,
    fail: function (e) {
...
onLoad(function (_options) {
  console.log(_options, app.globalData.storeInfo)
  const storeIdactive = uni.getStorageSync('storeIdactive')
  console.log('storeIdactive', storeIdactive)
  state.changeFlag = _options && _options.changeFlag ? _options.changeFlag : 0
  if (storeIdactive) {
    state.intoStoreInfo = storeIdactive
    state.storeId = storeIdactive.id
    state.store = storeIdactive.name
  }
  // 购物车数量
  const localProductCount = uni.getStorageSync('wj_userProductsCount')
  state.cartCount = localProductCount
  state.themeColor =
  ...
  console.log('当前地理位置获取最近门店')
  console.log(state.intoStoreInfo)
...
onShow(function () {
  ...
  const storeIdactive = uni.getStorageSync('storeIdactive')
  console.log('storeIdactive', storeIdactive)
  if (storeIdactive && state.changeFlag == 1) {
    state.intoStoreInfo = storeIdactive
    state.storeId = storeIdactive.id
    state.store = storeIdactive.name
  }
  ...
  if (state.storeId) {
    ...
    if (state.intoStoreInfo && state.storeId !== state.intoStoreInfo.id) {
      state.storeId = state.intoStoreInfo.id
      state.store = state.intoStoreInfo.name
      queryProductMenu()
  ...
  if (state.intoStoreInfo && state.storeId !== '' && state.changeFlag == 1) {
    state.store = state.intoStoreInfo.name
    state.storeId = state.intoStoreInfo.id
    ANALYSIS.PVStatistics(state.storeId)
    if (currentPage.options.categoryId != null) {
      getVerticalMenuProduct(state.storeId, currentPage.options.categoryId)
    } else {
    ...
    ADDRESS.getLocationStores()
      .then((res) => {
        console.log(res, 'dasdjlasjdlaksjl')
        // console.log('res195',res,app.globalData.storeInfo)
        ...
        // })
        if (res) {
          state.intoStoreInfo = res
          state.storeId = res.id
          state.store = res.name
        }
        uni.setStorageSync('storeIdactive', res)
        ANALYSIS.PVStatistics(state.storeId)
        if (currentPage.options.categoryId != null) {
          getVerticalMenuProduct(res.id, currentPage.options.categoryId)
        } else {
...
onUnload(function () {
  ...
  // app.globalData.storeInfo = this.data.intoStoreInfo
})
...
function addShopcart(val) {
  ...
  if (style !== 'SPEC_PARENT' && style !== 'MEALS2') {
    if (loginStatus) {
      storeId = state.intoStoreInfo.id
      let postData = {
        storeId,
        lists: [
          {
  ...
  if (app.globalData.userInfo && app.globalData.userInfo.member) {
    shopcartService
      ...
      .then((res) => {
        ...
        try {
          console.log('wj_shopCartStorage', localData, localGrabData)
...
function getVerticalMenuProduct(storeId, categoryId) {
  ...
  if (util.isDeliveryProduct()) {
    ...
    if (app.globalData.distributionStore) {
      ...
      //   console.log('是不是进入到了这里')
      // const distributionStore = app.globalData.distributionStore;
      // storeIds = storeIds + "," + distributionStore.id
      handleGetCategoryWithCountByStore(storeId)
    } else {
      storeService
        ...
        .then((res) => {
          if (res) {
            ...
            storeIds = storeIds + ',' + distributionStore.id
            handleGetCategoryWithCountByStore(storeIds)
            try {
  ...
  function handleGetCategoryWithCountByStore() {
    productService
      ...
      .then((res) => {
        ...
        function handleCategorys() {
          state.category = tempCategorys
          if (tempCategorys.length > 0) {
            ...
            if (!showCategorys) {
              if (level === 1) {
                showCategorys = tempCategorys[firstStage]
              } else if (level === 2) {
                if (tempCategorys[firstStage].children.length > 0) {
                  showCategorys = tempCategorys[firstStage].children[secondStage]
                }
              } else if (level === 3) {
                if (tempCategorys[firstStage].children.length > 0) {
                  if (tempCategorys[firstStage].children[secondStage].children.length > 0) {
                    showCategorys =
                      tempCategorys[firstStage].children[secondStage].children[thirdStage]
                  } else {
            ...
            if (showCategorys.parentId != null && level > 1) {
              if (level === 2) {
                ...
                if (!isNoSubcategories) {
                  currentCategory = tempCategorys[firstStage].categoryId
...
function clickCategoryItem(e) {
  ...
  console.log(thirdSelect, secondId)
  state.currenSelect = tempSelect
...
function bindgoShop() {
  console.log('点击购物车')
  uni.setStorageSync('wj_zhiti', 'zhiti')
  NAVPAGE.toShopcart()
}
...
<style scoped>
/* pages/mallModule/tabbar/selfLifting/selfLifting.wxss */
