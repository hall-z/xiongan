<template>
  ...
  <view>
    <view class="page-content" :style="state.goodsNumAll == 0 ? 'padding-bottom: 40rpx' : ''" v-if="!state.loading">
      <view
        v-if="(state.systemOptions.isDeliveryProduct || state.systemOptions.isNextDayProduct) && state.shoppingCartShowType != 'UD'">
        <view class="header-info">
          <view class="header-tab">
            <view :class="'nav-title ' + (state.tabSelected == 2 ? 'active' : '')"
              :style="'color: ' + state.themeColor + ';border: 3rpx solid ' + state.themeColor + ';border-right: 3rpx solid ' + state.themeColor + ';background: ' + (state.tabSelected == 2 ? state.themeColor : '') + ';'"
              data-idx="2" @click="clickCategory">{{state.shoppingData.nextDay.labelName || '次日达'}}
              <text :hidden="!state.shoppingData.nextDay.goodsNum">({{state.shoppingData.nextDay.goodsNum}})</text>
            </view>
            <view :class="'nav-title ' + (state.tabSelected == 1 ? 'active' : '')"
              :style="'color: ' + state.themeColor + ';border: 3rpx solid ' + state.themeColor + ';border-right: 3rpx solid ' + state.themeColor + ';background: ' + (state.tabSelected == 1 ? state.themeColor : '') + ';'"
              data-idx="1" @click="clickCategory">{{state.shoppingData.distribution.labelName || '商城商品'}}
              <text
                :hidden="!state.shoppingData.distribution.goodsNum">({{state.shoppingData.distribution.goodsNum}})</text>
            </view>
            <view :class="'nav-title ' + (state.tabSelected == 0 ? 'active' : '')"
              :style="'color: ' + state.themeColor + ';border: 3rpx solid ' + state.themeColor + ';border-left: 3rpx solid ' + state.themeColor + ';background: ' + (state.tabSelected == 0 ? state.themeColor : '') + ';'"
              data-idx="0" @click="clickCategory">{{state.shoppingData.store.labelName || '门店商品'}}
              <text :hidden="!state.shoppingData.store.goodsNum">({{state.shoppingData.store.goodsNum}})</text>
            </view>
          </view>
          <view class="edit" @click="handleDelete" :data-type="state.tabSelected == 1 ? 'distribution' : 'store'">
      ...
      <view class="shopcart-title">
        <view class="location" for="checkAll" @click="changeAddress">
          <image style="width:22rpx;height:30rpx" :src="state.imagesPath.iconNearStoreAddress"></image>
          <text>{{state.addressName || '暂无地址信息'}}</text>
        </view>
      </view>
      <!-- <view class="shopcart-title">
		<view class="location" for="checkAll" bindtap="changeStore">
			<image class="shop-img" src="{{state.imagesPath.iconStore}}" ></image>
			<text>{{state.storeName}}</text>
			<text hidden="{{state.tabSelected == 1 && state.shoppingCartShowType != 'UD'}}" style="color: {{state.themeColor}}">切换</text>
			<image class="right-img" src="{{state.imagesPath.orderRight}}" hidden="{{state.tabSelected == 1 && state.shoppingCartShowType != 'UD'}}" ></image>
    </view>
	</view> -->
      <view
        :class="'shopcart-main ' + (isIphoneX ? 'iphoneX-height' : '') + ' ' + (state.giftGoodsList.length > 0 ? 'hasGift' : '')"
        :hidden="state.goodsNumAll == 0 && state.expiredGoodsList.length == 0 ? true : false">
        <view class="shopcart-item-box" v-for="(shoppingTypeItem , index) in state.shoppingType"
          :hidden="!(state.shoppingData[shoppingTypeItem] && state.shoppingData[shoppingTypeItem].goodsList && state.shoppingData[shoppingTypeItem].goodsList.length > 0)">
          <view
            v-if="state.shoppingCartShowType == 'UD' && state.shoppingData[shoppingTypeItem] && state.shoppingData[shoppingTypeItem].goodsList && state.shoppingData[shoppingTypeItem].goodsList.length > 0"
            class="store-type-title">
            <checkbox-group @change="checkboxAllChange" :data-type="shoppingTypeItem" class="check-box">
              <label class="checkbox">
                <checkbox
                  :disabled="!(state.shoppingData[shoppingTypeItem] && state.shoppingData[shoppingTypeItem].goodsList && state.shoppingData[shoppingTypeItem].goodsList.length > 0)"
                  :value="shoppingTypeItem" :style="state.calcPromotionsFlag != 0 ? 'pointer-events: none' : ''"
                  :checked="state.shoppingData[shoppingTypeItem] && state.shoppingData[shoppingTypeItem].checkedAll"
                  :color="state.themeColor">
                  <!-- {{shoppingData[shoppingTypeItem].labelName}} -->
                  <text>{{state.shoppingData[shoppingTypeItem] && state.shoppingData[shoppingTypeItem].storeName}}</text>
                </checkbox>
              </label>
            </checkbox-group>
            <view class="edit"
              :hidden="!(state.shoppingData[shoppingTypeItem] && state.shoppingData[shoppingTypeItem].goodsList && state.shoppingData[shoppingTypeItem].goodsList.length > 0)"
              @click="handleDelete" :data-type="shoppingTypeItem">
              <image :src="state.imagesPath.iconShopCartDelete"></image>
            </view>
          </view>
        ...
        暂无数据~
          ...
          <checkbox-group @change="checkboxChange" :data-type="shoppingTypeItem">
            <view
              :class="'shopcart-item ' + (item && item.promotionDescription ? 'promotionDescription' : '') + ' ' + (item && item.status == 'DELETED' ? 'expiredGood' : '')"
              v-for="(item , index) in (state.shoppingData[shoppingTypeItem] && state.shoppingData[shoppingTypeItem].goodsList ? state.shoppingData[shoppingTypeItem].goodsList : [])"
              :key="index" v-if="item">
              <view class="goods-top"
                v-if="item && item.promotionsLabel && item.promotionsLabel.length > 0 || item && item.promotionDescription && item.discount">
                <text class="discount-icon" v-for="(labelItem , index) in item.promotionsLabel" :key="index"
                  :data-id="index" v-if="index < 3">{{labelItem.labelName}}</text>
                <view class="discount-desc">
                  {{item.promotionDescription && item.discount ? item.promotionDescription + ";" : item.promotionDescription}}{{item.discount ? '已优惠' + item.discount + '元' : ''}}
                </view>
              </view>
              <view class="goods-box">
                <checkbox :value="item.id" :style="state.calcPromotionsFlag != 0 ? 'pointer-events: none' : ''"
                  :checked="item.status === 'DELETED' ? false : item.checked" :color="state.themeColor">
                  <view class="goods-img-box" @click="toGoodsDetails" :data-type="shoppingTypeItem" :data-id="item.id">
                  ...
                  <view class="goods-info" @click="toGoodsDetails" :data-type="shoppingTypeItem" :data-id="item.id">
                    <view class="goods-name">
                      <view>{{item.name}}</view>
                    </view>
                    <view class="goods-detail-info" v-if="item.style === 'MEALS2'">
                      <text v-for="(groupItem , index) in item.groupProducts"
                        :key="index">{{groupItem.groupProductCount}}x{{groupItem.productName}}</text>
                    </view>
                    ...
                    <text class="goods-desc" v-if="item.deliveryTimeStatement"
                      style="color: #ff9f43;">{{item.deliveryTimeStatement}}</text>

                    <view v-if="item.require" class="require_label">
                      必选
                    </view>
                    <view class="goods-name produt-label">
                      <view v-for="(labelItem , index) in item.produtlabel" :key="index" v-if="index < 3">
                        <text :style="'background-color:rgb(' + labelItem.color + ')'"
                          v-if="labelItem.labelName && !labelItem.imageUrl">{{labelItem.labelName}}</text>
                        <image style="width: 80rpx" mode="widthFix" :src="labelItem.imageUrl"
                          v-else-if="labelItem.imageUrl"></image>
                      </view>
                    </view>
                    <view class="price-number">
                      <view class="price-box" v-if="item.sellPrice >= 0 && item.sellPrice != null && item.sellPrice">
                      ...
                      <view class="count-box">
                        <view :data-id="item.id" class="iconfont icon-jianhao" :data-type="shoppingTypeItem"
                          @click.stop="handleReduce"
                          :style="'color:' + ((item.productNum || 0) > 1 ? state.themeColor : '#BBBBBC') + ';'"
                          :hidden="item.status === 'DELETED' || item.balance <= 0"></view>
                        <text :data-id="item.id" :data-type="shoppingTypeItem"
                          :hidden="item.status === 'DELETED' || item.balance <= 0"
                          @click.stop="handleNumber">{{item.productNum || 0}}</text>
                        <view :data-id="item.id" :data-type="shoppingTypeItem" class="iconfont icon-jiahao2fill"
                          @click.stop="handleAdd" :style="'color: ' + state.themeColor + ';'"
                          :hidden="item.status === 'DELETED' || item.balance <= 0"></view>
                      </view>
                    </view>
                  </view>
                  <view class="bg-shade" v-if="item.status === 'DELETED' || item.balance <= 0">
                    <text class="delete-btn" :data-id="item.id" :data-traceId="item.traceId"
                      @click="clickDelete">清除</text>
                  </view>
                </checkbox>
              </view>
              <view class="gift-box" v-if="item.goodsPromotions && item.goodsPromotions.length > 0">
                ...
                <view class="gift-item-box">
                  <view class="gift-item" v-for="(giftItem , index) in item.goodsPromotions" :key="i">
                    <view>
        ...
        <view class="expiredGoods" :hidden="state.expiredGoodsList.length == 0 ? true : false"
          ...
          <view class="expiredGoodTitle">
            <view class="expiredGood-title-box">
              ...
              <view class="custom-btn" @click="changeisExpand">
                <image :src="state.imagesPath.upIcon" :class="state.showExpiredGood ? 'up-icon rotate-icon' : 'up-icon'"
                  mode=""></image>
              </view>
            </view>
            <text class="clearExpiredGood" @click="clearExpiredGood" style="color:#999">清空失效商品</text>
          </view>
          <checkbox-group @change="expiredBoxChange" v-if="state.showExpiredGood">
            <view class="shopcart-item" v-for="(item , index) in state.expiredGoodsList" :key="index">
              <view class="goods-box">
                <checkbox :value="item.id" :disabled="true">
                  ...
                  <view class="goods-info" @click="toGoodsDetails" :data-id="item.id">
                    ...
                    <view class="goods-name">
                      <view v-for="(expiredLabelItem , index) in item.produtlabel" :key="index" v-if="index < 3">
                        <text :style="'background-color:rgb(' + expiredLabelItem.color + ')'"
                          v-if="expiredLabelItem.labelName && !expiredLabelItem.imageUrl">{{expiredLabelItem.labelName}}</text>
                        <image style="width: 80rpx" mode="widthFix" :src="expiredLabelItem.imageUrl"
                          v-else-if="expiredLabelItem.imageUrl"></image>
                      </view>
                    ...
                    <view class="price-number">
                      <view class="price-box"
                        v-if="item.originalPrice || item.sellPrice >= 0 && item.sellPrice !== undefined && item.sellPrice">
                        <!-- <text>￥</text>{{item.sellPrice}} -->
                        <!-- <text class="old-price" wx:if="{{item.originalPrice && item.sellPrice < item.originalPrice}}">￥{{item.originalPrice}}</text> -->
                      </view>
                      <view class="count-box">
      ...
      <view :class="'gift-box gift-bottom-box ' + (isIphoneX ? 'iphoneX-height' : '')"
        v-if="state.giftGoodsList.length > 0">
        <view class="gift-title">赠品:</view>
        <view class="gift-item-box">
          <view class="gift-item">
            <view class="gift-img-box" v-for="(item , idx) in state.giftGoodsList" :key="i" v-if="idx < 4">
      ...
      <view class="no-data" :hidden="state.goodsNumAll == 0 && state.expiredGoodsList.length == 0 ? false : true">
        <image :src="state.imagesPath.imgNoDataShopcart"></image>
        <button size="mini" @click="toHome"
          :style="'color: ' + state.themeColor + ';border:1px solid ' + state.themeColor">随便逛逛</button>
      </view>
      <!-- 门店精选 -->
  ...
  >
      ...
      <view :class="'money-detail ' + (isIphoneX ? 'iphoneX-height' : '')"
        ...
        <view class="money-detail-pop" v-if="state.show.money">
          ...
          <view class="pop-content-data" v-for="(typeItem , index) in state.shoppingType"
            :hidden="!(state.shoppingData[typeItem] && state.shoppingData[typeItem].selectShow)">
            <view class="pop-sum flex-between">
              <view>{{state.shoppingData[typeItem] && state.shoppingData[typeItem].labelName}}合计</view>
              <view>￥{{state.shoppingData[typeItem] && state.shoppingData[typeItem].sellPrice}}</view>
            </view>
          </view>
          ...
          <view class="cash-discount flex-between"
        ...
        <view :class="'bottom-info ' + (isIphoneX ? 'iphoneX-height' : '')" v-if="state.goodsNumAll > 0"
          :style="'height:' + (state.navigationStyle == '1' ? '128rpx' : '98rpx') + ';padding-bottom:' + (state.navigationStyle == '1' ? '68rpx' : '38rpx') + ';'">
          <checkbox-group @change="checkboxAllChange" class="check-box">
            <label class="checkbox" for="checkAll">
              <checkbox value="all" :style="state.calcPromotionsFlag != 0 ? 'pointer-events: none' : ''"
                :checked="state.checkedAll" :color="state.themeColor"> 全选
              </checkbox>
            </label>
          </checkbox-group>
          <view class="calc-info">
            <view>合计:
              ...
              <text class="delivery-fee" v-if="state.baseFare > 0">(另需{{state.baseFare}}元配送费)</text>
            </view>
            <!-- <view class="discount-box" catchtap="doShowMoneyDetail">共减:￥{{discount}} | 明细 </view> -->
            <view class="discount-box" @click.stop="doShowMoneyDetail">共减:￥{{state.shoppingDiscount}} | 明细 </view>
            <view v-if="state.deliveryRulesText">
              <text :class="state.deliveryRulesText.length > 17 ? 'fz20' : ''">{{state.deliveryRulesText}}</text>
            </view>
          </view>
          <text class="tocalc" :style="'background-color: ' + state.themeColor" @click="pretreatmentOrder">去结算</text>
        </view>
      </view>
      <view
        :class="'recommend-block ' + (isIphoneX ? 'iphoneX-height' : '') + ' ' + (state.giftGoodsList.length > 0 ? 'hasGift' : '')"
        v-if="state.recommendingNum > 0"></view>
    </view>
    <custom-loading v-if="state.loading || state.calcPromotionLoading"></custom-loading>

    <popup :show="state.show.separateSettlement" position="bottom" custom-class="middle"
      ...
      <view class="money-detail-pop">
        ...
        <view :class="'pop-content-data pop-content-data-settele ' + (index == 1 ? 'content-border' : '')"
          ...
          <view class="pop-sum flex-between">
            <view>
              <view>共 {{state.shoppingData[typeItem] && state.shoppingData[typeItem].goodTotalNum}}
                件商品，小计<text>￥{{state.shoppingData[typeItem] && state.shoppingData[typeItem].allPrice}}</text></view>
              <view class="price-tip">不包含运费、包装费</view>
            </view>
            <view @click.stop="handlePlaceOrder" class="settle-btn" :data-type="typeItem"
              :style="'background-color: ' + state.themeColor">去结算</view>
          </view>
        </view>
      </view>
    </popup>
          ...
          <view class="content">
            <view class="count-box">
              ...
              <input cursor="0" selection-start="1" selection-end="1" :value="state.goodsInfo.productNum"
                @input="bindProductNumInput" type="number" maxlength="4">
              <view @click.stop="handleProductNumAdd" :hidden="item.status === 'DELETED' || item.balance <= 0">+</view>
            </view>
          </view>
          <view class="button-box">
            <button class="cancel" :data-id="state.goodsInfo.id" @click.stop="onCancel">取消</button>
            <button class="confirm" :style="'background: ' + state.themeColor" :data-id="state.goodsInfo.id"
              @click.stop="onConfirm">确认</button>
          </view>
        </view>
      </view>
    </popup>
    <popup :show="state.show.middle" position="middle" custom-class="middle" :overlayStyle="state.overlayStyle"
    ...
    <popup :show="state.show.oneMore" position="middle" custom-class="middle" @close="toggleOneMorePopup">
      <view class="oneMore-content">
        <view class="oneMore-content-box">
          ...
          <scroll-view scroll-y="" class="unsupported-product-box">
            <view class="unsupported-product-item" v-for="(item , index) in unsupportedProductList" :key="id">
              <view class="name">{{item.productName}}</view>
              <view class="number">x<text>{{item.quantity}}</text></view>
    ...
    <tabbar :tabbar="state.tabbar" :show="state.showTabbar" @popup="recorderPopup" v-if="!state.openCustomTabbar">
    </tabbar>
...
const state = reactive({
  ...
  storeName: '人民广场店',
  tabSelected: 0,
  distributionStoreId: uni.getStorageSync('wj_distributionStore') ? uni.getStorageSync('wj_distributionStore').id : '',
  distributionStoreName: uni.getStorageSync('wj_distributionStore') ? uni.getStorageSync('wj_distributionStore').name : '',
  nextDayStoreId: uni.getStorageSync('wj_nextDayStore') ? uni.getStorageSync('wj_nextDayStore').id : '',
  nextDayStoreName: uni.getStorageSync('wj_nextDayStore') ? uni.getStorageSync('wj_nextDayStore').name : '',
  goodsNum: {
    storeNum: 0,
    distributionNum: 0,
    nextDayNum: 0
...
function getShopCartStyle () {
  let keys = ['storeNameAs', 'distributionNameAs', 'nextDayNameAs', 'storeNameSort', 'distributionNameSort', 'nextDayNameSort', 'shoppingCartShowType'];
...
function handleSync (postData) {
  ...
  function shopCartSync (postData) {
    shopcartService.update(postData).then(res => {
      ...
      shopcartService.getCatering(state.activeShoppingType || state.activeStoreId).then(res1 => {
        let res = res1[0];
        console.log(res);
        let supportGoodsList = [];
        ...
        supportGoodsList.forEach((ele, index) => {
          if (ele.id === tempList[index].id) {
            ...
            if (ele.discounts && ele.discounts.cashDiscount && ele.discounts.totalDiscount > 0 && ele.discounts.cashDiscount.description && ele.discounts.cashDiscount.description != null && ele.discounts.cashDiscount.description != "null") {
              promotionDescription = promotionDescription + ele.discounts.cashDiscount.description + ";";
            }
            if (ele.itemLabels && ele.itemLabels.length > 0) {
              ele.itemLabels.forEach(item => {
                if (item && item.type === 'GOODSLABEL' && item.color != null) {
                  ...
                  if (item.labelDetails != null && promotionDescription.indexOf(item.labelDetails) < 0 && item.labelDetails !== ",") {
                    promotionDescription = promotionDescription + item.labelDetails + ";";
                  }
                }
              });
            }
            promotionDescription = promotionDescription.substring(0, promotionDescription.length - 1);
            let nameArr = ele.name.split("x");
            let specs = '';
            let name = ele.name;
            if (nameArr.length > 0) {
        ...
        if (state.activeShoppingType === 'store') {
          let goodList = state.shoppingData[state.activeShoppingType].goodsList;
          let tempShopCart = {};
          let grabProducts = {};
...
function handlePriceCalc (e) {
  const that = this;
  let storeType = e && e.currentTarget.dataset.type;
  // let tempList = []
  let checkedNum = 0;
  let storeId = '';
  ...
  state.shoppingType.forEach(shoppingTypeItem => {
    ...
    let checkedFlag = state.shoppingData[shoppingTypeItem].goodsList.some(item1 => {
...
function handleReduce (e) {
  if (isAdd) {
    ...
    let tempData = state.shoppingData[storeType].goodsList;
    let tempLists = [];
    let productNum = 0;
    ...
    if (productNum > 1 && (!currentItem.minQty || productNum > currentItem.minQty) && (!currentItem.initialPurchaseNumber || productNum > currentItem.initialPurchaseNumber)) {
...
function handleProductNumReduce () {
  let goodsInfo = state.goodsInfo;
  let num = goodsInfo.productNum;
  let minQty = goodsInfo.minQty;
  let initialPurchaseNumber = goodsInfo.initialPurchaseNumber;
  if (num > 1 && (!minQty || num > minQty) && (!initialPurchaseNumber || num > initialPurchaseNumber)) {
    num--;
    console.log(num);
    state.goodsInfo = {
      ...goodsInfo,
...
function onCancel (e) {
  const that = this;
  let tempData = state.shoppingData[state.activeShoppingType].goodsList;
  tempData.forEach(item => {
    if (item.id === e.currentTarget.dataset.id) {
...
function onConfirm (e) {
  ...
  let nowGoodsInfo = state.goodsInfo;
  let tempLists = [];
  let minQty = nowGoodsInfo.minQty;
  let initialPurchaseNumber = nowGoodsInfo.initialPurchaseNumber;
    ...
    if (num <= nowGoodsInfo.balance) {
      if (nowGoodsInfo.secKillInfo) {
        if (num >= nowGoodsInfo.secKillInfo.limitCount) {
          util.showToast('亲，' + nowGoodsInfo.name + '只可以秒杀' + nowGoodsInfo.secKillInfo.limitCount + '个，不可以贪心哦~');
          state.goodsInfo = {
            ...nowGoodsInfo,
        ...
        if (nowGoodsInfo.orderCount) {
          if (num >= nowGoodsInfo.orderCount) {
            util.showToast('亲，' + nowGoodsInfo.name + '每单限购' + nowGoodsInfo.orderCount + '个哦~');
            state.goodsInfo = {
              ...nowGoodsInfo,
  ...
  if (tempLists.length > 0) {
    ...
    if (state.activeShoppingType == 'nextDay' || state.activeShoppingType == state.nextDayStoreId) {
      storeId = state.nextDayStoreId;
    } else if (state.activeShoppingType == 'distribution' || state.activeShoppingType == state.distributionStoreId) {
      storeId = state.distributionStoreId;
    } else {
      storeId = isNumber(state.activeShoppingType) ? state.activeShoppingType : app.globalData.storeInfo.id;
    }
    let postData = {
      storeId: storeId,
      ...tempGoodsData
...
function getGoodDetailData () {
  state.shoppingType.forEach(typeItem => {
    ...
    state.shoppingData[typeItem].goodsList.forEach(item => {
      if (item) {
        let goodAllPrice = item.sellPrice && item.productNum ? parseFloat(parseFloat(item.sellPrice * (item.productNum || 0)).toFixed(2)) : 0;
...
function checkboxChange (e) {
  ...
  let storeType = e.currentTarget.dataset.type;
  let num = state.shoppingData[storeType].goodsList.length || 0;
  ...
  let checkedAll = true;
  state.shoppingType.forEach(shoppingTypeItem => {
    if (!state.shoppingData[shoppingTypeItem].checkedAll && state.shoppingData[shoppingTypeItem].goodsList && state.shoppingData[shoppingTypeItem].goodsList.length > 0) {
      checkedAll = false;
    }
  });
...
function expiredBoxChange (e) {
  console.log(e);
  let tempData = state.expiredGoodsList;
  tempData.forEach(ele => {
    ele.checked = true;
  });
  e.detail.value.forEach(item => {
...
function checkboxAllChange (e) {
  ...
  let checkedAll = true;
  state.shoppingType.forEach(shoppingTypeItem => {
    if (!state.shoppingData[shoppingTypeItem].checkedAll && storeType && state.shoppingData[shoppingTypeItem].goodsList && state.shoppingData[shoppingTypeItem].goodsList.length > 0) {
      checkedAll = false;
    }
    if (!storeType) {
    ...
    if (!storeType || shoppingTypeItem == storeType) {
      let tempData = state.shoppingData[shoppingTypeItem].goodsList;
...
async function calcPromotions (postData, allPrice, storeType) {
  ...
  const that = this;
  let keys = Object.keys(state.shoppingData);
  let tempAllPrice = parseFloat(allPrice).toFixed(2);
  // wx.showLoading({})
  state.calcPromotionLoading = true;
  await promotionService.bathCalcPromotions(postData).then(res => {
    let allPrice1 = 0;
    let totalDiscount = 0;
    res.forEach(item => {
      let goods = [];
    ...
    function getPromotionsResult (res, goods) {
      ...
      let giftGoods = [];
      if (res && res.promotions && res.promotions.goodsPromotions && res.promotions.goodsPromotions.length > 0) {
        // 有促销且促销中有送商品促销
...
function getPrice () {
  ...
  state.shoppingType.forEach(storeTypeItem => {
    allPrice += state.shoppingData[storeTypeItem].allPrice ? parseFloat(state.shoppingData[storeTypeItem].allPrice) : 0;
    discount += state.shoppingData[storeTypeItem].discount ? parseFloat(state.shoppingData[storeTypeItem].discount) : 0;
  });
...
async function queryShopcart () {
  ...
  async function getShopCartData () {
    await shopcartService.getCatering(state.activeStoreId).then(res => {
      console.log('获取购物车数据=====');
      console.log(res);
      state.shoppingType = [];
      if (res && res.length > 0) {
      ...
      function getShopCartData (res, storeType) {
        ...
        let tempItems = [];
        let oldGoodsList = state.shoppingData[storeType].goodsList;
        ...
        res.items.forEach((ele, index) => {
          ...
          ele = {
            ...
            name: name,
            specs: specs,
            goodsPromotions: goodsPromotions,
            produtlabel: produtlabel,
            promotionsLabel: promotionsLabel,
          ...
          if (ele.status !== 'DELETED') {
            tempItems.push({
              ...ele,
              checked: ele.storeBalance === 0 ? false : true
            });
            checkedCount++;
          }
        });
        ...
        if (expiredGoodsList && expiredGoodsList.length > 0 && tempItems && tempItems.length > 0) {
          expiredGoodsList.forEach(expiredGood => {
            let flag = tempItems.some(tempGood => {
        ...
        if (checkedCount === tempItems.length - expiredGoodNum) {
          ...
          state.allPrice = allPrice.toFixed(2);
          state.shoppingData = state.shoppingData;
          state.goodsNumAll = state.shoppingCartShowType == 'UD' ? tempItems.length || state.goodsNumAll : tempItems.length;
...
function queryAllShopcart (storeId) {
  let number = shopcartService.getStoreProductsCount(storeId).then(res => {
    let num = 0;
    if (res && res.length > 0) {
      num = res[0].quantity;
    }
...
let handlePlaceOrder = util.throttle(function (e) {
  const that = this;
  if (state.calcPromotionsFlag != 0) {
    uni.showToast({
      icon: 'loading'
  ...
  let storeType = e && e.currentTarget.dataset.type ? e.currentTarget.dataset.type : state.activeShoppingType;
  toPerfectOrder(storeType);
});
function toMultipleGoodPerfectOrder (selectTypeList) {
  const that = this;
  let availableGoodsListAll = [];
  ...
  if (selectTypeList && selectTypeList.length > 0) {
    selectTypeList.forEach((storeType, storeTypeIndex) => {
      ...
      if (tempGoodsList.length > 0) {
        try {
          let shopcart = {
            allPrice: (parseFloat(state.shoppingData[storeType].allPrice) + parseFloat(state.shoppingData[storeType].discount)).toFixed(2),
          ...
          let existIndex = null;
          let existFlag = shopcartList.some((shopcartItem, shopcartIdx) => {
...
function handlePageLoad (refresh = true) {
  ...
  if (parameter.orderId != null && parameter.from === "oneMore") {
    ...
    function oneMore (storeId) {
      ...
      orderService.oneMore(postData).then(res => {
        if (res && res.length > 0) {
          // 有商品不能加入购物车需要报错
...
onLoad(async function (_options) {
  ...
  systemOptions.forEach(item => {
    ...
    if (item.key === 'isDeliveryProduct' && item.value != null && item.value !== '') {
      item.value === 'TRUE' ? configures.isDeliveryProduct = true : configures.isDeliveryProduct = false;
    } else if (item.key === 'isNextDayProduct' && item.value != null && item.value !== '') {
      // 是否包含次日达商品:TRUE是、FALSE否
      item.value === 'TRUE' ? configures.isNextDayProduct = true : configures.isNextDayProduct = false;
    }
  });
  ...
  if (configures.isDeliveryProduct && state.shoppingCartShowType != 'UD') {
    ...
    function setDistributionStoreInfo (res) {
      state.distributionStoreId = res.id;
      state.distributionStoreName = res.name;
      let tab = app.globalData.storeIdentification === 'distribution' ? state.shoppingType.indexOf('distribution') : 0;
    ...
    // 查询次日达门店信息
    ADDRESS.getNextDayStore().then(res => {
      state.isOnload = false;
    ...
    function setNextDayStoreInfo (res) {
      state.nextDayStoreId = res.id;
      state.nextDayStoreName = res.name;
      let tab = app.globalData.storeIdentification === 'nextDay' ? state.shoppingType.indexOf('nextDay') : 0;
      state.tabSelected = tab;
...
onReady(function () { });
function getAddress () {
  let chooseAddress = uni.getStorageSync('wj_chooseAddressInfo');
    ...
    uni.getLocation({
      ...
      success: function (res) {
        ...
        util.inverseAnalysisAddress(latitude, longitude).then(res => {
          ...
          state.addressName = resData.formatted_address ? resData.formatted_address : resData.formattedAddresses.recommend;
        });
      }
    });
  }
...
onShow(function () {
  ...
  if (!state.isOnload) {
    if (app.globalData.isShopCartChange) {
      ...
      let tab = app.globalData.storeIdentification === 'distribution' || app.globalData.storeIdentification === 'nextDay' ? state.shoppingType.indexOf(app.globalData.storeIdentification) : 0;
      state.tabSelected = tab;
      handlePageLoad();
...
onShareAppMessage(async function () {
  ...
  if (!sharePictures) {
    let indexPic = await util.getSharePictures('SHOPPING_CART');
    if (indexPic) {
  ...
  if (app.globalData.userInfo && app.globalData.userInfo.member) {
    ...
    let memberName = app.globalData.userInfo.member.nickName;
    let memberAvatar = app.globalData.userInfo.member.avatar;
    let goodsList = [
      // ...that.data.shoppingData.store.goodsList,
      // ...that.data.shoppingData.distribution.goodsList
    ];
    state.shoppingType.forEach(key => {
      goodsList = goodsList.concat(state.shoppingData[key] && state.shoppingData[key].goodsList ? state.shoppingData[key].goodsList : []);
    });
    console.log(goodsList, 'that.data.shoppingData');
    let expiredGoodsList = JSON.stringify(state.expiredGoodsList);
    let productList = [];
    ...
    let path = `/pages/mallModule/shareShopcart/shareShopcart?shareId=${shareId}`;
    return {
...
function getSharePictures () {
  ...
  //   })
  // }
  util.getSharePictures('SHOPPING_CART').then(res => {
    if (res) {
...
function toGoodsDetails (e) {
  ...
  if (product.grabActivityId != null && product.grabActivityId != "") {
    let opts = '?productId=' + product.productId + '&storeId=' + storeId + '&activityId=' + product.grabActivityId + '&type=secondkill';
    ...
    let opts = '';
    if (storeType == 'store') {
      opts = '?productId=' + (product.parentProductId ? product.parentProductId + '&specId=' + product.productId : product.productId) + '&goodsDetail=' + goodsDetail + '&type=newmbr' + '&zhiti=zhiti';
    } else {
      opts = '?productId=' + (product.parentProductId ? product.parentProductId + '&specId=' + product.productId : product.productId) + '&goodsDetail=' + goodsDetail + '&type=newmbr';
    }
    NAVPAGE.toGoodsDetails(opts);
    //普通商品
  } else {
    ...
    let opts = '';
    if (storeType == 'store') {
      opts = '?productId=' + (product.parentProductId ? product.parentProductId + '&specId=' + product.productId : product.productId) + '&storeId=' + storeId + '&type=normal' + '&zhiti=zhiti';
    } else {
      opts = '?productId=' + (product.parentProductId ? product.parentProductId + '&specId=' + product.productId : product.productId) + '&storeId=' + storeId + '&type=normal';
    }
    NAVPAGE.toGoodsDetails(opts);
  }
}
...
function checkDeliveryFeeRulesDisplay () {
  ...
  function handleDeliveryFeeRule () {
    ...
    if (rules) {
      let takeoutFare = rules.takeoutFare || 0;
      let freeFare = rules.freeFare || 0;
      if (allPrice < takeoutFare) {
        distanceExemptionAmount = parseFloat((takeoutFare - allPrice).toFixed(2));
        deliveryRulesText = takeoutFare > 0 ? "差" + distanceExemptionAmount.toFixed(2) + "元起送" : "";
        ...
        if (rules.deliveryPartFreeFee) {
          // 有部分满减
          let deliveryPartFreeFeeFreeFare = rules.deliveryPartFreeFee.freeFare || 0;
          let deliveryPartFreeFeeSatisfyFare = rules.deliveryPartFreeFee.satisfyFare || 0;
          if (deliveryPartFreeFeeSatisfyFare > 0 && deliveryPartFreeFeeFreeFare > 0) {
            // 已满足起送条件
            if (allPrice < deliveryPartFreeFeeSatisfyFare) {
              deliveryRulesText = "满" + deliveryPartFreeFeeSatisfyFare + "元减" + deliveryPartFreeFeeFreeFare + "元配送费";
              if (freeFare > 0) {
