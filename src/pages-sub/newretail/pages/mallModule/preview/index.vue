<template>
  ...
  <navigationBar
    ...
    :background="titlebarBackgroundColour || '#fff'"
  ...
  <view
    v-if="state.hasaAuthorization && previewId"
  ...
  <navigator
    ...
    v-if="
      state.hasaAuthorization && state.pageShow && state.liveRoomShowCenter && state.roomId !== -1
    "
  >
    <image
      ...
      mode=""
    />
    <image v-if="state.liveRoomLogoUrl" :src="state.liveRoomLogoUrl" />
  </navigator>
  <view v-if="!state.hasaAuthorization" class="no-authorization">
  ...
  >
    <view class="popup-adsense-box">
      <view class="popup-unvisited-coupon-content">
        <image :src="state.imagesPath.iconUnreadCouponTop" mode="widthFix"></image>
        <scroll-view class="unvisited-coupon-board" :scroll-y="true">
          ...
          >
            <image
              ...
              :src="state.imagesPath.iconUnreadCouponIten"
              mode="widthFix"
            ></image>
            <!-- 价格 -->
        ...
        <view class="popup-unvisited-coupo-bottom" @click="toMyCoupon">
          <view>立即查看></view>
        </view>
      </view>
    </view>
  ...
  >
    <view
      :class="'popup-official-account-content-top ' + (state.isPushCollectShowed ? 'collect' : '')"
    ...
    <view
      ...
      "
    ></view>
    <view :class="'popup-official-account-content ' + (state.isPushCollectShowed ? 'collect' : '')">
...
function handleIndexPopup() {
  ...
  //   return;
  // }
  console.log('getNewMemberAdgetNewMemberAd')
  // 微信已经不允许强制用户授权
  // 判断是否开启极速登录
...
onLoad(async function (_options) {
  state.previewId = _options.scene + ''
  console.log('onLoad.........')
  const self = this
  ...
  if (!state.onPullDownRefresh && _options.isReLaunch != null) {
  ...
  if (!app.globalData.storeInfo || app.globalData.storeId !== '') {
    ...
    ADDRESS.getLocation()
      .then((res) => {
        ...
        const tempAllStores = uni.getStorageSync('wj_allStores') // 获取缓存的所有门店
    ...
    ADDRESS.getStoreDistance(app.globalData.storeInfo).then((res) => {
...
function getLoginDonateCouponAdsense(storeId) {
  const that = this
  if (queryLoginDonateCoupon === true) {
    return
  }
  queryLoginDonateCoupon === true
  let hasShow = false
  ...
  if (hasShow) {
    queryLoginDonateCoupon === false
    // 有弹窗在显示
    return
  }
  if (state.onPullDownRefresh === false) {
    try {
...
onPageScroll(function (e) {
  ...
  if (state.isHomeShowTopSearch) {
    ...
    if (e.scrollTop > searchHeight && !state.isSearchFixed) {
      state.isSearchFixed = true
    } else if (e.scrollTop <= searchHeight && state.isSearchFixed) {
