<template>
<!--pages/mallModule/activity/lottery/helpLottery/helpLottery.wxml-->
<view>
<navigationBar :title="state.navigationBarTitle"></navigationBar>
<!-- 支付完分享 -->
<view class="top-info share" v-if="state.type === 'share' && state.loading">
  <image class="top-bg" :src="state.imagesPath.iconPeneryShare20191106" mode="widthFix" lazy-load="false"></image>
  ...
  <view class="share-box">
    <view class="penny-goods-info" @click="toLotteryIndex">
      <view class="penny-goods-img">
        <image :src="rechargeDrawActivity.activityImageUrl" mode="widthFix"></image>
      </view>
      <view>
        <text class="name">{{rechargeDrawActivity.name}}</text>
    ...
    <view class="button-box">
      <text v-if="!state.isInTheOpeningAward">邀请{{rechargeDrawActivity.minHelperQuantity}}位好友参与，就能获得中奖资格</text>
      <text v-if="state.isInTheOpeningAward && state.instance.helperQuantity < state.instance.minHelperQuantity">您未获得中奖资格</text>
      <text v-if="state.isInTheOpeningAward && state.instance.helperQuantity >= state.instance.minHelperQuantity">您已获得中奖资格</text>
      <button class="share-btn" v-if="state.isInTheOpeningAward">开奖中</button>
      <button class="share-btn" @click="toggleSharePopup" v-else-if="state.instance.helperQuantity < state.instance.minHelperQuantity">立即获取抽奖资格({{state.instance.helperQuantity}}/{{state.instance.minHelperQuantity}})</button>
      <view v-else>
        <text>已获取抽奖资格，去提高中奖概率</text>
        <view class="hasQualifications">
          <button class="racharge-btn" @click="recharge" style="background: #fff; border: 1rpx solid #e02e24; color: #e02e24;">充值获得助力点</button>
          <button class="share-btn" @click="toggleSharePopup">邀请好友助力</button>
        </view>
      </view>
      <!-- <text>已有{{instance.helperQuantity}}人帮您助力,助力越多中奖几率越大</text> -->
      <view class="button-tips">
        <text class="left">已获得{{state.instance.helpPoint}}助力点</text>
        <text class="right" @click="checkHelpPointDetails">查看助力点排行</text>
      </view>
      <text class="activity-rules">助力点数=助力总次数*充值总金额</text>
    </view>
  </view>
</view>
<!-- 助力 -->
<view class="top-info help" v-if="state.type === 'help' && state.loading">
  <image class="top-bg" :src="state.imagesPath.iconLotteryHelp20191025" mode="widthFix" lazy-load="false" v-if="state.isHelp"></image>
    ...
    <view class="goods-info">
      ...
      <view class="info">
        ...
        <view class="price-num">
          <view class="num">共抽
            <text>{{rechargeDrawActivity.prizeQuantity}}</text>件</view>
        </view>
      </view>
    </view>
  </view>
  <view class="button-box">
    <view v-if="state.instance.status === 'JOINED' && state.isHelp">
      <text>助力成功！</text>
      <button v-if="state.isInTheOpeningAward">开奖中</button>
      <button @click="toLotteryIndex" v-else-if="!state.hasPenny">我也要抢</button>
      <button @click="toMyHelpLotteryDetails" v-else>查看我的抽奖</button>
    </view>
    <view v-else-if="state.instance.status === 'JOINED' && !state.isHelp">
    ...
    <view v-else>
      ...
      <button @click="toLotteryIndex">立即抢其他奖品</button>
    </view>
  </view>
</view>
<!-- 结果 -->
<view class="top-info result" v-if="state.type === 'result' && state.loading">
  ...
  <view class="result-box">
    <view class="goods-name">{{rechargeDrawActivity.name}}</view>
    <view class="result-text" v-if="state.instance.status === 'JOINED' || state.instance.status === 'NOPRIZED'">未中奖，试试其他商品吧</view>
    <view class="result-text" v-else>恭喜您中奖啦！</view>
    <view class="more-penny" @click="toLotteryIndex">立抢更多奖品</view>
    <view class="button-tips">
      <text class="left" @click="checkWinningDetails">查看中奖详情</text>
      <text class="right" @click="checkHelpPointDetails">查看助力点排行</text>
    </view>
  </view>
</view>
...
<view class="poster-box" v-if="state.showPoster" @click="handlePosterCancel" @touchmove.stop="preventTouchMove">
  <view class="poster-box-bg">
    ...
    <button @click.stop="handleSaveToImg" class="saveImage" :style="'background: ' + themeColor">保存图片到相册</button>
...
<popup :show="state.show.bottom" position="bottom" custom-class="bottom" @close="toggleSharePopup">
  <view class="penny-share-box">
    ...
    <view class="share-button-box">
      <text>邀请好友助力</text>
      <text>才能获得中奖资格</text>
      <view class="share-button-all">
        <view class="share-button-item">
          <button open-type="share" @click="subscriptionMessagePopup">
            <image class="share-people-bg" :src="state.imagesPath.iconPenneryWeChet"></image>
          </button>
          <text class="share-tips">分享给好友助力</text>
        </view>
        <view class="share-button-item">
          <button @click="handleShareTimeline">
            <image class="share-circle-bg" :src="state.imagesPath.iconFriendsCircle"></image>
          </button>
          <text class="share-tips">分享微信朋友圈助力</text>
        </view>
      </view>
    </view>
  </view>
</popup>
<popup :show="state.show.bottom2" position="bottom" custom-class="bottom" @close="toggleShare">
  <view class="penny-share-box">
    <view class="title">大奖只差最后一步！</view>
    <view class="share-button-box">
      <text>邀请规定数量的好友助力</text>
      <text>才能获得中奖资格</text>
      <button class="share-btn-bg-btn" open-type="share" @click="subscriptionMessagePopup">
        <image class="share-btn-bg" :src="state.imagesPath.iconPenneryWeChet"></image>
      </button>
      <text class="share-tips">分享到微信群聊</text>
    </view>
  </view>
</popup>
...
<popup :show="state.show.helpPoint" position="middle" custom-class="middle" @close="toggleHelpPointPopup">
  <view class="penny-helpPoint-box">
    <view class="penny-helpPoint-popup">
      <view class="title">
        <image class="left-icon" :src="state.imagesPath.iconPeneryLine"></image>
        <text>助力点排行</text>
        <image class="right-icon" :src="state.imagesPath.iconPeneryLine"></image>
      ...
      <view class="close">
        <image :src="state.imagesPath.iconCloseImg2" @click="toggleHelpPointPopup"></image>
      </view>
      <scroll-view class="people-list" scroll-y="false">
        <view class="people-item" v-for="(item , index) in state.helpPointList" :key="id">
          <view>
            <image :src="item.avatar || state.imagesPath.userPortrait" mode="widthFix"></image>
            <view class="member-info">
              <text>{{item.creatorName}}</text>
      ...
      <view class="penny-helpPoint-tips">只显示前20名的排行，排行榜每一分钟更新一次</view>
    </view>
  </view>
</popup>
  ...
  <view :class="'popup-penny ' + (!state.hasCoupon ? 'noCoupon' : '')">
    <view class="popup-penny-box" v-if="state.hasCoupon">
      ...
      <view class="bottom-button-box">
        ...
        <view class="penny" @click="toLotteryIndex">我也要抢</view>
      </view>
    </view>
    <view class="popup-penny-box noCoupon" v-else>
      ...
      <view class="bottom-button-box">
        <view class="coupon" @click="toLotteryIndex">我也要抢</view>
      </view>
    </view>
...
// pages/mallModule/activity/lottery/helpLottery/helpLottery.js
...
function initPage() {
  ...
  if (options.instanceId) {
    getRechargeDrawInstance(options.instanceId);
  }
  if (options.scene) {
    const instanceId = decodeURIComponent(options.scene);
    getRechargeDrawInstance(instanceId);
  }
}
...
onShareAppMessage(function (res) {
  if (res.from === 'button') {
    ...
    let imgUrl = state.rechargeDrawActivity.shareImageUrl;
    const name = state.rechargeDrawActivity.name;
    let memberId = "";
    ...
    return {
      ...
      path: '/pages-sub/newretail/pages/mallModule/activity/lottery/helpLottery/helpLottery?instanceId=' + instanceId + "&shareId=" + memberId,
      imageUrl: imgUrl,
      success(e) {
        uni.showShareMenu({
...
function getRechargeDrawInstance(id) {
  const self = this;
  lotteryService.getInstanceById(id).then(res => {
    ...
    if (type === "share") {
      type = res.status === "JOINED" ? "share" : "result";
    }
    // 不是本人检查是否已经助力
    state.instance = instance;
    state.type = type;
    state.isCreator = isCreator;
    if (!isCreator && res.status === "JOINED") {
      checkIsHelp();
    }
    if (!res.address && res.status === "JOINED" && isCreator && customData.isCreate && !customData.isShowAddress) {
      // 如果是支付完成创建实例进入页面，实例中没有地址信息，弹出更新地址弹窗，弹窗只弹一次
      checkDistribution();
    }
    return res;
  }).then(res => {
    ...
    // 处理活动信息
    const _data = res.activity;
    let hasCoupon = false;
    if (_data.helperCouponActivityId) {
      hasCoupon = true;
    }
    // 处理充值抽奖活动倒计时
    state.rechargeDrawActivity = _data;
    ...
    // 处理未中奖送券
    state.loading = true;
    if (state.instance.status === "NOPRIZED" && state.instance.unprizedCouponActivityId && state.instance.unprizedCouponActivityId !== "") {
      hasCoupon = true;
      state.hasCoupon = hasCoupon;
      state.couponType = "NOPRIZED";
      togglePennyPopup();
    }
    // 设置进度
    ...
    function countDown() {
      ...
      if (countDownArr[0].day === "00" && countDownArr[0].hou === "00" && countDownArr[0].min === "00" && countDownArr[0].sec === "00") {
        if (state.instance.status === "JOINED") {
...
function help(e) {
  ...
  lotteryService.help(postData).then(res => {
    state.isHelp = true;
    state.hasCoupon && togglePennyPopup();
    getRechargeDrawInstance(state.instance.id);
  }).catch(err => {
    // 特殊处理助力次数已达上限
    state.disabledHelp = false;
    if (err.code === 55010) {
...
function checkIsHelp() {
  ...
  lotteryService.hasHelp(state.instance.id, app.globalData.userInfo.member.id).then(res => {
    state.isHelp = res;
  }).catch(err => {
    UTILS.showToast(err.message);
  });
...
function queryMyInstance() {
  ...
  if (!customData.hasMyInstanceQueryCount) {
    ...
    if (app.globalData.userInfo && app.globalData.userInfo.member) {
      if (app.globalData.userInfo.member.id === state.instance.memberId) {
        let isCreator = true;
        let type = isCreator ? "share" : "help";
        if (type === "share") {
          type = state.instance.status === "JOINED" ? "share" : "result";
        }
        state.type = type;
        state.isCreator = isCreator;
        return;
...
function checkWinningDetails() {
  ...
  const postData = {
    ...
    statusIn: ["PRIZED"],
    orderDirection: "desc",
    orderField: "helper_quantity"
  };
  lotteryService.queryInstance(postData).then(res => {
    if (res && res.records && res.records.length >= 0) {
      const tempList = res.records;
...
function checkHelpPointDetails() {
  if (!(app.globalData.userInfo && app.globalData.userInfo.member)) {
    // 没有会员信息
    return;
  }
  if (state.helpPointList.length > 0) {
    toggleHelpPointPopup();
    return;
  }
  const that = this;
  const postData = {
    ...
    statusIn: ["JOINED", "PRIZED", "NOPRIZED"],
    orderDirection: "desc",
    orderField: "help_point"
  };
  lotteryService.queryInstance(postData).then(res => {
    if (res && res.records && res.records.length >= 0) {
      const tempList = res.records;
      state.helpPointList = tempList;
      toggleHelpPointPopup();
    }
  }).catch(err => {
    UTILS.showToast(err.message);
...
function recharge() {
  uni.navigateTo({
    url: '/pages-sub/newretail/pages/mallModule/activity/lottery/lotteryIndex/lotteryIndex'
  });
}
function handleShareTimeline(e) {
  ...
  // 绘制上部分商品图片
  let sharePicture = "";
  if (state.instance.shareImageUrl) {
    sharePicture = state.instance.shareImageUrl.replace("http://", "https://");
  } else {
    sharePicture = state.instance.activityImageUrl.replace("http://", "https://");
  }
  uni.getImageInfo({
    src: sharePicture,
    success(res) {
  ...
  ctx.fillText('长按识别小程序码', 110 * rate, 434 * rate);
  ctx.draw(true);

  // 获取小程序码
  let appid = request.APP_ID;
  let page = 'pages/mallModule/activity/lottery/helpLottery/helpLottery';
  let scene = options.instanceId;
  ...
  function createSharePicture(appid, page, scene, width) {
    ...
    uni.getImageInfo({
      ...
      success(res) {
        console.log(res, '二维码');
        ctx.drawImage(res.path, 110 * rate, 330 * rate, 90 * rate, 90 * rate);
        ctx.draw(true);
      },
      fail() {
...
function togglePromptPopup(e) {
  ...
  if (state.show.prompt && (!e || e.currentTarget.dataset.type !== "cancel")) {
    toggleShare();
  }
  toggle('prompt');
}
...
function handleSave(e) {
  ...
  if (!state.userName) {
    ...
    if (app.globalData.userInfo && app.globalData.userInfo.member) {
      ...
      UTILS.addressResolution(defaultAddress).then(res => {
        if (res && res.data && res.data.location) {
          ...
          let postData = {
            ...
            // 收货地址： 省区
            receiverMobile: state.telNumber,
            // 收货人手机
            receiverName: state.userName // 收货人名称
          };
          lotteryService.updateAddress(postData).then(res => {
            // 保存地址
      ...
      //     // 新增地址
      //     let postData = {
      //       address: self.data.detailInfo, // 详细收货街道地址
      //       city: self.data.region[1], // 收货地址： 市区
      //       county: self.data.region[2], // 收货地址： 县区
      //       instanceId: self.data.instance.id, // 抽奖实例ID
      //       province: self.data.region[0], // 收货地址： 省区
      //       receiverMobile: self.data.telNumber, // 收货人手机
      //       receiverName: self.data.userName, // 收货人名称
      //     }
      //     lotteryService.updateAddress(postData).then(res => {
      //       // 保存地址
      //       UTILS.showToast("保存成功~")
...
<style scoped>
/* pages/mallModule/activity/lottery/helpLottery/helpLottery.wxss */
...
.share-box .button-box .hasQualifications {
  display: flex;
  justify-content: space-between;
  width: 100%;
}

.share-box .button-box .hasQualifications button {
  width: 300rpx;
  height: 90rpx;
  font-size: 36rpx;
  line-height: 90rpx;
...
.share-box .button-box .button-tips {
  font-size: 30rpx;
  line-height: 36rpx;
  color: #676767;
  display: flex;
...
.share-box .button-box .button-tips text {
  padding: 0 20rpx;
  margin: 0;
}

.share-box .button-box .button-tips text.left {
  color: #676767;
}

.share-box .button-box .button-tips text.right {
  margin-left: 1px;
  position: relative;
}

.share-box .button-box .button-tips text.right::before {
  content: "";
  display: block;
  position: absolute;
  left: -1px;
...
.result-box .button-tips text {
  padding: 0 20rpx;
  margin: 0;
}

.result-box .button-tips text.left {
  color: #676767;
}

.result-box .button-tips text.right {
  margin-left: 1px;
  position: relative;
}

.result-box .button-tips text.right::before {
  content: "";
  display: block;
  position: absolute;
  left: -1px;
...
.share-button-box .share-btn-bg-btn {
  width: 110rpx;
  height: 110rpx;
  border-radius: 50%;
  display: flex;
...
.share-button-box .share-btn-bg-btn .share-btn-bg {
  width: 78rpx;
  height: 60rpx;
}

.share-button-box .share-button-all {
  display: flex;
  align-items: center;
}

.share-button-box .share-button-all .share-button-item {
  flex: 1;
}

.share-button-box .share-button-all .share-button-item text {
  font-size: 32rpx;
  line-height: 66rpx;
  color: #1b1b1b;
  display: block;
...
.share-button-box .share-button-all .share-button-item button {
  width: 110rpx;
  height: 110rpx;
  border-radius: 50%;
  display: flex;
...
.share-button-box .share-button-all .share-button-item button .share-people-bg {
  width: 78rpx;
  height: 60rpx;
}

.share-button-box .share-button-all .share-button-item button .share-circle-bg {
  width: 70rpx;
  height: 70rpx;
}

.share-button-box .share-button-all .share-button-item text.share-tips {
  font-size: 26rpx;
  line-height: 46rpx;
  color: #47484a;
}
...
.penny-helpPoint-popup .title image {
  width: 98rpx;
  height: 4rpx;
}

.penny-helpPoint-popup .title image.left-icon {
  transform: rotate(180deg);
  margin-right: 18rpx;
}

.penny-helpPoint-popup .title image.right-icon {
  margin-left: 18rpx;
}

.penny-helpPoint-popup text {
  display: block;
  margin-bottom: 12rpx;
}

.penny-helpPoint-popup .close {
  width: 58rpx;
  height: 58rpx;
  position: absolute;
  right: -29rpx;
...
.penny-helpPoint-popup .close image {
  width: 40rpx;
  height: 40rpx;
}

.penny-helpPoint-tips {
  text-align: center;
}
