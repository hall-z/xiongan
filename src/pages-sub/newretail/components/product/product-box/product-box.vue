<template>
  ...
  <view
    ...
    v-if="
      props.bannerJson &&
      props.bannerJson.imageItems &&
      props.bannerJson.imageItems[0].imageUrl &&
      props.isNew === '1'
    "
    :key="index"
  ...
  >
    <image
      v-for="(item, index) in props.bannerJson.imageItems"
      :key="index"
      :src="item.imageUrl"
      mode="widthFix"
      style="width: 100%"
  ...
  <view :class="'goods-content ' + props.externalClass" v-if="props.show && !props.zhiti">
    <view :class="'goods-con-box ' + (props.proShowType === 'card' ? 'goods-con-box-card' : '')">
      <block v-for="(item, index) in state.goodsList" :key="index">
        <view v-if="state.goodsList.length > 0 && props.proShowType === 'list'">
          <product-item
            :goods="item"
            :showSpecParent="props.showSpecParent"
            :shopCart="props.shopCart"
            :hasUserInfo="props.hasUserInfo"
            @onClickGoods="clickGoodsItem"
            @addToCart="addToCart"
            @noop="noop"
      ...
      <block v-for="(item, index) in state.goodsList" :key="index">
        <view v-if="state.goodsList.length > 0 && props.proShowType === 'card'">
          <product-item-card
            :goods="item"
            :showSpecParent="props.showSpecParent"
            :shopCart="props.shopCart"
    ...
    <view v-if="state.goodsList.length == 0" class="no-data">
      <image :src="state.imagesPath.imgNoData"></image>
    </view>
    <button class="loading-btn" :loading="state.showLoading" v-if="state.showLoading">
      正在加载...
    </button>
    <text class="no-more-goods" v-if="props.noMore">~去看看其他分类吧~</text>
  </view>
  <view :class="'goods-content ' + props.externalClass" v-if="props.show && props.zhiti">
    <view :class="'goods-con-box ' + (props.proShowType === 'card' ? 'goods-con-box-cardA' : '')">
      <block v-for="(item, index) in state.goodsList" :key="index">
        <view v-if="state.goodsList.length > 0 && props.proShowType === 'list'">
          <product-item
            :goods="item"
...
import productItem from '@/pages-sub/newretail/components/product/goods-item-right/goods-item-right.vue'
...
function bannerJumping(e) {
  ...
  const bannerList = props.bannerJson.imageItems
