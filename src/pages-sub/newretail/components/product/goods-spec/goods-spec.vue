 ...
 <view class="spec-top-box">
    <view class="spec-img-box">
      ...
      <view class="spec-close-box" @click="handleClose">
        ...
        <image :src="imagesPath.category_close" mode="widthFix"></image>
      </view>
      <view class="price-box">
        <view class="price" :style="'color: ' + themeColor" v-if="!goodSelect">
          <!-- {{isTeamPrice ? '拼团价' : ''}}
          {{isGrabPrice ? '秒杀价' : ''}}
          {{isBargainPrice ? '最低可砍至' : ''}}
          {{isSolitairePrice ? '接龙价' : ''}}
          {{!type && promotionPrice && (!sellPrice || promotionPrice < sellPrice) && (!memberPrice || promotionPrice < memberPrice) ? '促销价' : ''}}
          {{!type && memberPrice && memberPrice < sellPrice ? vipGradeConfig.priceLabel : ''}} -->
          ￥<text>{{minSellPrice}}</text>{{minSellPrice == maxSellPrice ? '' : '-'}}<text v-if="minSellPrice < maxSellPrice">{{maxSellPrice}}</text>
        </view>
  ...
  <scroll-view class="spec-con-box" scroll-y="" :style="'min-height:310rpx;max-height: 580rpx;height:' + specsItem.specParams.length * 60 + 'rpx;'">
    <view class="spec-section" v-for="(specsItem , col) in allSpecs" :key="i">
      <view class="spec-section-name">{{specsItem.name}}</view>
      <view class="spec-option">
        <view :class="'spec-option-item ' + (item.isSelect ? 'active' : '') + ' ' + (item.disabled ? 'disabled' : '')" :style="'background: ' + (item.isSelect ? themeColor : '')" v-for="(item , opt) in specsItem.specParams" :key="j" :data-label="item.value" :data-specId="item.specId" :data-id="item.id" :data-item="opt" :data-disabled="item.disabled" @click="clickOptionItem" :hidden="item.status != 'ON'">
  ...
  <view class="choose-product-num">
    ...
    <view class="goods-count-box">
      <view @click.stop="handleReduce" class="iconfont icon-jianhao" :style="'color: ' + themeColor + ';border-color: ' + themeColor + ';'"></view>
      <view class="num">
        <input cursor="0" selection-start="1" selection-end="1" :value="productNum" @input="bindProductNumInput" type="number" maxlength="3">
      </view>
      <view @click.stop="handleAdd" class="iconfont icon-jiahao2fill" :style="'color: ' + themeColor + ';'"></view>
    </view>
  </view>
  <view class="spec-button-box">
    <view class="spec-button-box-content">  
      <form @submit="confirm" @click.stop="noop">
        <button v-if="activityType === 'normal'" form-type="submit" class="spec-button" :style="'background: ' + themeColor" data-addtype="detail">确定</button>
        <button v-else-if="activityType === 'secondkill'" form-type="submit" :class="'spec-button ' + (!isGrabPrice && isImmediatelyGrab ? 'disabled' : '')" :style="'background: ' + themeColor" data-addtype="detail">确定</button>
        <button v-else-if="activityType === 'group'" form-type="submit" :class="'spec-button ' + (!isTeamPrice ? 'disabled' : '')" :style="'background: ' + themeColor" data-addtype="detail">确定</button>    
        <button v-else-if="activityType === 'bargain'" form-type="submit" :class="'spec-button ' + (!isBargainPrice ? 'disabled' : '')" :style="'background: ' + themeColor" data-addtype="detail">确定</button>   
        <button v-else-if="activityType === 'solitaire'" form-type="submit" :class="'spec-button ' + (!isSolitairePrice ? 'disabled' : '')" :style="'background: ' + themeColor" data-addtype="detail">确定</button>     
      </form>
    </view>
  </view>
</view></view>
...
function clickOptionItem(e) {
  ...
  let selArr = [];
  // 规格选中项的下标数组所有
  let specSelArr = new Array(tempSpecs.length);
  for (let i = 0; i < tempSpecs.length; i++) {
    specSelArr[i] = null;
  }
  ...
  const specDetails = state.specDetails;
  ...
  if (select == tempSpecs.length) {
    ...
    for (let i = 0; i < optionalProductArr.length; i++) {
      ...
      let difference = getArrDifference(selArr, ele);
          ...
          if (specDetails && specDetails.length > 0) {
            flag = specDetails.some(spec => {
              if (Array.isArray(spec)) {
                return spec.some(spec1 => {
                  if (Array.isArray(spec1)) {
                    return spec1.some(spec2 => {
                      let specFlag = spec2.specParamIds.some(specId => {
                        return specId == item.id && spec2.balance > 0 && spec2.status == 'ON';
                      });
                      return specFlag;
                    });
                  } else {
                    let specFlag = spec1.specParamIds.some(specId => {
                ...
                let specFlag = spec.specParamIds.some(specId => {
  ...
  let price = state.price.toFixed(2);
  ...
  if (_data2.sellPrice != null) {
    ...
    if (state.isTeam) {
      let num = _data2.teamMemberJoinCount || 1;
    ...
    if (state.select.balance > 0) {
      if (state.activityType !== 'normal') {
        ...
        if (state.isSolitaire) {
          ...
          if (state.productNum > state.select.balance || state.productNum > state.select.solitaireBalance) {
            ...
            if (state.select.balance > state.select.solitaireBalance) {
              if (state.productNum > state.select.balance) {
                // 提示门店库存不足
              } else if (state.productNum > state.select.solitaireBalance && state.productNum < state.select.balance) {
                // 提示活动库存不足
...
function handleSelectData() {
  ...
  if (_data3.sellPrice != null) {
    ...
    if (state.isTeam) {
      let num = _data3.teamMemberJoinCount || 1;
      let groupPrice = price;
...
function handleAdd() {
  ...
  if (canAdd) {
    if (productNum >= state.select.balance || productNum >= state.select.solitaireBalance) {
      if (state.activityData && state.activityData.memberLimit > 0 && state.activityData.memberLimit <= productNum) {
        ...
        if (state.select.balance > state.select.solitaireBalance) {
          if (productNum >= state.select.balance) {
            // 提示门店库存不足
          } else if (productNum >= state.select.solitaireBalance && productNum < state.select.balance) {
            // 提示活动库存不足
...
watch(() => props.goodsImg, (newVal, oldVal) => {
  state.imageUrl = newVal;
});

watch(() => props.activeSpecId, (newVal, oldVal) => {
  
});

watch(() => props.specs, (newVal, oldVal) => {
        ...
        newVal.forEach(ele => {
          ...
          if (ele.status === 'ENABLE') {
            ...
            ele.specParams.forEach(item => {
              let flag = false;
              let specStatus = 'OFF';
              if (state.specDetails && state.specDetails.length > 0) {
                flag = state.specDetails.some(spec => {
                  if (Array.isArray(spec)) {
                    return spec.some(spec1 => {
                      if (Array.isArray(spec1)) {
                        return spec1.some(spec2 => {
              ...
              if (state.activeSpecId) {
                let selectFlag = state.specDetails.some(spec => {
                  if (Array.isArray(spec)) {
                    return spec.some(spec1 => {
                      if (Array.isArray(spec1)) {
                        return spec1.some(spec2 => {
                          let specFlag = spec2.specParamIds.some(specId => {
                            return specId == item.id && state.activeSpecId == spec2.productId;
                          });
                          if (specFlag) {
                            state.select = spec2;
...
watch(() => props.specDetails, (newVal, oldVal) => {
...
watch(() => props.price, (newVal, oldVal) => {
  console.log(newVal, state.sellPrice, 'newValnewVal');
        // 属性被改变时执行的函数（可选），也可以写成在methods段中定义的方法名字符串, 如：'_propertyChange'
        // 通常 newVal 就是新设置的数据， oldVal 是旧数据
...
watch(() => props.memberPrice, (newVal, oldVal) => {
  // 属性被改变时执行的函数（可选），也可以写成在methods段中定义的方法名字符串, 如：'_propertyChange'
        // 通常 newVal 就是新设置的数据， oldVal 是旧数据
...
watch(() => props.promotionPrice, (newVal, oldVal) => {
  // 属性被改变时执行的函数（可选），也可以写成在methods段中定义的方法名字符串, 如：'_propertyChange'
        // 通常 newVal 就是新设置的数据， oldVal 是旧数据
        if (newVal < state.sellPrice || !state.sellPrice) {
...
watch(() => props.hasSelected, (newVal, oldVal) => {
  const self = this;
        if (newVal === true) {
          let tempSpecs = state.allSpecs;
          let selectText = '已选："';
...
watch(() => props.activityType, (newVal, oldVal) => {
  if (newVal === "group") {
          state.isTeam = true;
          ...
          let sellPrice = state.sellPrice ? parseFloat(state.sellPrice) : state.sellPrice;
          if (state.isSecondkill) {
            state.specDetails.forEach(item => {
...
watch(() => props.hasTeamLeader, (newVal, oldVal) => {
  // 属性被改变时执行的函数（可选），也可以写成在methods段中定义的方法名字符串, 如：'_propertyChange'
        // 通常 newVal 就是新设置的数据， oldVal 是旧数据
        state.isTeamLeader = newVal;
});

watch(() => props.hasImmediatelyGrab, (newVal, oldVal) => {
  // 属性被改变时执行的函数（可选），也可以写成在methods段中定义的方法名字符串, 如：'_propertyChange'
        // 通常 newVal 就是新设置的数据， oldVal 是旧数据
        state.isImmediatelyGrab = newVal;
});

watch(() => props.productNum, (newVal, oldVal) => {
  // 属性被改变时执行的函数（可选），也可以写成在methods段中定义的方法名字符串, 如：'_propertyChange'
        // 通常 newVal 就是新设置的数据， oldVal 是旧数据
        if (newVal === 0) {
          state.productNum = 1;
...
watch(() => props.minQty, (newVal, oldVal) => {
