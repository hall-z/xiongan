...
<view class="select-date">
    <view class="inner-box">
        ...
        <view class="select-day">
            <view class="day" v-for="(item , index) in selfData" :key="index" @click="getnowDay" :data-index="index">{{item}}</view>
        </view>
        <view class="select-time">
            <!-- <view v-for="(item,index) in deliveryTimeList" :key="index"> -->
            <view v-if="nowDay === 0">
                <view v-if="deliveryTimeList[nowDay].timeList.length > 0">
                    <view :class="index === selectTimeIndex && nowDay === selectDay ? 'select-time-item select' : 'select-time-item'" v-for="(item , index) in deliveryTimeList[nowDay].timeList" @click="clickTime" :key="index" :data-index="index">
                        {{item}}
                    </view>
                </view>
                <view v-else class="time-title">抱歉，没有可选{{title}}
                </view>
            </view>
            <view v-if="nowDay !== 0">
                <view :class="index === selectTimeIndex && nowDay === selectDay ? 'select-time-item select' : 'select-time-item'" v-for="(item , index) in deliveryTimeList[nowDay].timeList" @click="clickTime" :key="index" :data-index="index">
...
function clickTime(e) {
  let selfData = state.selfData[state.nowDay];
  let index = e.currentTarget.dataset.index;
  let selectTime = state.deliveryTimeList[state.nowDay].timeList[index];
  let deliveryTime = selfData + ' ' + selectTime;
  let nowDay = state.nowDay;
...
watch(() => props.deliveryTimeData, (newVal, oldVal) => {
