...
<!-- <view hidden="{{!showBox}}" class="{{mode=='pop'?'mask':''}}">
  <view class="{{mode=='pop'?'verifybox':''}}">
    <block wx:if="{{mode=='pop'}}" style="{{'max-width:'+  ($root.m0 + 30+'px')+';'}}">
    ...
    <view class="verifybox-bottom" style="{{'padding:'+(mode=='pop'?'15px':'0')+';'}}">
      <block wx:if="{{componentType=='VerifySlide'}}">
        <view>
        </view>
      </block>
    </view>
...
<!-- bind:success="handleSuccess" bind:error="handleError" -->
<verify-slide :opt="captchaOpt1" class="vue-ref" :captchaType="captchaType" :type="verifyType" :figure="figure" :arith="arith" :width="width" :height="height" :fontSize="fontSize" :codeLength="codeLength" :mode="mode" :vOffset="vOffset" :vSpace="vSpace" :explain="explain" :imgSize="imgSize" :blockSize="blockSize" :barSize="barSize" :defaultNum="defaultNum" :checkNum="checkNum" data-ref="instance"></verify-slide>
</template>
<script setup>
...
ready(function () {
  const that = this;
  let m0 = parseInt(properties.imgSize.width);
  let g0 = Math.ceil(parseInt(properties.imgSize.width) * 50 / 310);
  let m1 = parseInt(properties.imgSize.height);
  state.$root = {
    m0: m0
  };
});
...
watch(() => props.captchaType, (newVal, oldVal) => {
...
watch(() => props.type, (newVal, oldVal) => {
  if (type) {
          switch (type.toString()) {
            case 'picture':
              state.verifyType = '1';
              state.componentType = 'VerifyCode';
              break;
            case '1':
              state.verifyType = '1';
              state.componentType = 'VerifyCode';
...
watch(() => props.show, (newVal, oldVal) => {
  self = this;
        console.log(newVal);
        state.showBox = newVal;
        if (newVal) {
          selectComponent('.vue-ref').hide();
          selectComponent('.vue-ref').show();
        }
});

</script>
<style scoped>
/* components/verify/verify.wxss */
.verifybox {
  position: relative;
