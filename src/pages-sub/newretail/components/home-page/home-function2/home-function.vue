<template>
  ...
  <view
    ...
    "
    v-if="state.functionEntryList.length > 0 && state.loaded"
  >
    <view
      class="column-item"
      v-for="(item, index) in state.functionEntryList"
      :key="index"
      :data-route="item.route"
      :data-item="item"
      :data-linkModel="item.linkModel"
      @click="activityJumping2"
    >
      <view class="column-img">
        <image lazy-load="" :src="item.imageUrl" mode="widthFix"></image>
      </view>
      <view :style="'font-size: 22rpx;margin-top: 10rpx;' + (colour ? 'color: ' + colour : '')">
        {{ item.modelName || '' }}
      </view>
    </view>
  </view>
  <view v-if="state.functionEntryList.length > 0"></view>
...
function getHomeFunctionEntry() {
  ...
  homepageService
    ...
    .then((res) => {
      ...
      const newArr = [].concat(tempFunctionList) // newArr为一个新的数组，不是简单的赋值引用
...
function handlePagePath(linkModel, data) {
  if (linkModel === 'grab_activity') {
    ...
    return (
      '/pages-sub/newretail/pages/mallModule/tabbar/category/category-list/category-list?categoryId=' +
      data.productMenuId
    )
      ...
      wxaUserService
        ...
        .then((res) => {
          ...
          if (!res.member) {
            ...
            return (
              '/pages-sub/newretail/pages/mallModule/activity/scoreRaffle/scoreRaffle?raffleActivityId=' +
              data.raffleActivityId
            )
          }
          uni.hideLoading()
        })
    ...
    return (
      '/pages-sub/newretail/pages/mallModule/goods/packaging/packaging?packagingId=' +
      data.packagingId
    )
  } else if (linkModel === 'tourism_plan') {
    return (
      '/pages-sub/newretail/pages/mallModule/benefitPlan/tourismPlan/tourismPlan?id=' +
      data.tourismPlanId
    )
  }
}
  ...
  (newVal, oldVal) => {
    ...
    arr.forEach((item) => {
      if (item.imageUrl) {
        ...
        // })
        item.linkType = 'MERCHANT_CATEGORY'
        item.pageParmas = '?storeTypeName=' + item.modelName + '&storeTypeId=1'
        item.route = item.linkModel ? handlePagePath(item.linkModel, item) : '' // 配置页面跳转
      }
    })
...
.column-box3 .column-item {
  width: 33%;
}
.column-box3 .column-item:nth-of-type(-n + 3) {
  margin-bottom: 0;
}
.column-box4 .column-item {
  width: 25%;
}
.column-box4 .column-item:nth-of-type(-n + 4) {
  margin-bottom: 0;
}
