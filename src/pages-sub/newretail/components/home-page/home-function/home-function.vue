<template>
  ...
  <view
    class="column-box"
    :style="functionEntryList.length > 5 ? 'justify-content: flex-start;' : ''"
    v-if="loaded && functionEntryList.length > 0"
  >
    <view
      class="column-item"
      v-for="(item, index) in functionEntryList"
      :key="index"
      :data-route="item.route"
      :data-linkModel="item.linkModel"
      @click="clickFunctionEntry"
    >
  ...
  <view v-if="functionEntryList.length > 0"></view>
...
function handlePagePath(linkModel) {
  if (linkModel === 'grab_activity') {
    ...
    return (
      '/pages-sub/newretail/pages/mallModule/goods/packaging/packaging?packagingId=' + packagingId
    )
