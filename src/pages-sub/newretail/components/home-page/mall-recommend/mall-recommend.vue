...
<view class="store-title" v-if="fineQualityGoodsList.length > 0 && isShowProductAreaTitle === 'TRUE'">
  <text>商城精选</text>
</view>
<view class="goods-box" :style="'display: ' + (uiconfig.displayStyle === 'two' || uiconfig.displayStyle === 'TWO' ? 'flex' : '')">
  <view v-if="uiconfig.displayStyle === 'one' || uiconfig.displayStyle === 'ONE'" v-for="(item , index) in fineQualityGoodsList" :key="idx">
    <goods-item :goods="item" :shopCart="shopCart" :hasUserInfo="hasUserInfo" @onClickGoods="onClickGoods" @addToCart="addToCart" @noop="noop" @getUserInfo="onGetUserInfo"></goods-item>
  </view>
  <view v-if="uiconfig.displayStyle === 'two' || uiconfig.displayStyle === 'TWO'" v-for="(item , index) in fineQualityGoodsList" :key="idx">
    <goods-store-hot :goods="item" :shopCart="shopCart" :hasUserInfo="hasUserInfo" @onClickGoods="onClickGoods" @addToCart="addToCart" @noop="noop" @getUserInfo="onGetUserInfo"></goods-store-hot>
  </view>
</view>
</template>
<script setup>
...
function getStoreProduct(storeId) {
  ...
  productService.getStoreHotProduct(storeId).then(res => {
    ...
    if (res && res.length > 0) {
      ...
      state.fineQualityGoodsList = newTempList;
      state.isShow = true;
    } else {
    ...
    console.log(state.fineQualityGoodsList, "商城精选");
  }).catch(error => {
    state.isShow = true;
    uni.showToast({
      title: error.message,
