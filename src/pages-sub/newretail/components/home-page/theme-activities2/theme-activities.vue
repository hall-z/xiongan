...
<view v-for="(item , i) in dataList" :key="idx" v-if="dataList.length > 0 && loaded">
  
    <!-- 普通主题 -->
    <view>
        <view class="module-box" :style="'margin: ' + moduleSpacing + 'px 0'" v-if="item.imageItems.length > 0 || item.productsForPc.length > 0">
            ...
            <!-- 单图 -->
            <view class="activity-img" v-if="item.prdouctDisplayStyle === 'SINGLE' && item.imageItems.length > 0">
                <view v-for="(item , idx) in item.imageItems" :key="idx">
                    <image lazy-load="" :src="item.imageUrl" mode="widthFix" v-if="idx < 1" :data-id="item.id" @click="activityJumping2" :data-item="item"></image>
                    <view class="swiper_activity_box" v-if="item.linkType == 'URL' && item.slideshowProduct == 'Y'">
                        <swiper @click.stop="activityJumping2" :data-item="item" :data-id="item.id" :style="'padding: ' + (item.spacingT || 0) + 'px ' + (item.spacingR || 0) + 'px ' + (item.spacingB || 0) + 'px ' + (item.spacingL || 0) + 'px'" :circular="true" :vertical="item.direction == 'about' ? false : true" :autoplay="true" :interval="3000">
                            <view v-for="(itemNum , idx) in urlSwiperObj[item.id].numberList" :key="idx">
                                <swiper-item @click.stop="activityJumping2" :data-item="item" :data-id="item.id">
                                    <view v-if="urlSwiperObj[item.id].arrImages && urlSwiperObj[item.id].arrImages[itemNum]">
                                        <image lazy-load="" mode="aspectFit" :src="urlSwiperObj[item.id].arrImages[itemNum]"></image>
                                    </view>
                                    <view v-if="urlSwiperObj[item.id].arrImages && urlSwiperObj[item.id].arrImages[itemNum + 1]">
                                        <image lazy-load="" mode="aspectFit" :src="urlSwiperObj[item.id].arrImages[itemNum + 1]"></image>
                                    </view>
                                </swiper-item>
                            </view>
                        </swiper>
                    </view>
                    <view class="swiper_activity_box" v-if="item.linkType != 'URL' && item.appReturnType == 'MIAOSHA' && item.slideshowProduct == 'Y'">
                        <swiper @click.stop="activityJumping2" :data-item="item" :data-id="item.id" :style="'padding: ' + (item.spacingT || 0) + 'px ' + (item.spacingR || 0) + 'px ' + (item.spacingB || 0) + 'px ' + (item.spacingL || 0) + 'px'" :circular="true" :vertical="item.direction == 'about' ? false : true" :autoplay="true" :interval="3000">
                            <view v-for="(itemNum , idx) in counterSwiperObj[item.grabActivityIdList[item.grabActivityIdList.length - 1]].numberList" :key="idx">
                                <swiper-item @click.stop="activityJumping2" :data-item="item" :data-id="item.id">
                                    <view v-if="counterSwiperObj[item.grabActivityIdList[item.grabActivityIdList.length - 1]].grabData1.products && counterSwiperObj[item.grabActivityIdList[item.grabActivityIdList.length - 1]].grabData1.products[itemNum]">
                                        <image lazy-load="" mode="aspectFit" :src="counterSwiperObj[item.grabActivityIdList[item.grabActivityIdList.length - 1]].grabData1.products[itemNum].imageUrl"></image>
                                    </view>
                                    <view v-if="counterSwiperObj[item.grabActivityIdList[item.grabActivityIdList.length - 1]].grabData1.products && counterSwiperObj[item.grabActivityIdList[item.grabActivityIdList.length - 1]].grabData1.products[itemNum + 1]">
                                        <image lazy-load="" mode="aspectFit" :src="counterSwiperObj[item.grabActivityIdList[item.grabActivityIdList.length - 1]].grabData1.products[itemNum + 1].imageUrl"></image>
                                    </view>
                                </swiper-item>
                            </view>
                        </swiper>
            ...
            <!-- 一行多张 -->
            <view v-if="(item.prdouctDisplayStyle === 'TWO_PER_LINE' || item.prdouctDisplayStyle === 'THREE_PER_LINE' || item.prdouctDisplayStyle === 'TIVE_PER_LINE') && item.imageItems.length > 0">
                <view :class="'activity-img ' + (item.prdouctDisplayStyle === 'TWO_PER_LINE' ? 'two-per-line-img' : '') + ' ' + (item.prdouctDisplayStyle === 'THREE_PER_LINE' ? 'three-per-line-img' : '') + ' ' + (item.prdouctDisplayStyle === 'TIVE_PER_LINE' ? 'three-per-line-img' : '')" :style="'width: 100%;padding: 0 ' + (pageMargin ? pageMargin * 2 + 2 : 0) + 'rpx;box-sizing: border-box'">
                    <view v-for="(item , idx) in item.imageItems" :key="idx" :style="'overflow: hidden;border-radius: ' + fillet + 'px;margin: ' + modelMargin + 'px;width:auto;'">
                        <image lazy-load="" :src="item.imageUrl" :style="'height: auto;width:' + (item.imageWidth ? 'calc(' + item.imageWidth + 'rpx + 1px)' : '100%')" mode="widthFix" :data-id="item.id" @click="activityJumping2" :data-item="item"></image>
                        <view class="swiper_activity_box" v-if="item.linkType == 'URL' && item.slideshowProduct == 'Y'">
                            <swiper @click.stop="activityJumping2" :data-item="item" :data-id="item.id" :style="'padding: ' + (item.spacingT || 0) + 'px ' + (item.spacingR || 0) + 'px ' + (item.spacingB || 0) + 'px ' + (item.spacingL || 0) + 'px'" :circular="true" :vertical="item.direction == 'about' ? false : true" :autoplay="true" :interval="3000">
                                <view v-for="(itemNum , idx) in urlSwiperObj[item.id].numberList" :key="idx">
                                    <swiper-item @click.stop="activityJumping2" :data-item="item" :data-id="item.id">
                            ...
                            <swiper @click.stop="activityJumping2" :data-item="item" :data-id="item.id" :style="'padding: ' + (item.spacingT || 0) + 'px ' + (item.spacingR || 0) + 'px ' + (item.spacingB || 0) + 'px ' + (item.spacingL || 0) + 'px'" :circular="true" :vertical="item.direction == 'about' ? false : true" :autoplay="true" :interval="3000">
                                <view v-for="(itemNum , idx) in counterSwiperObj[item.grabActivityIdList[item.grabActivityIdList.length - 1]].numberList" :key="idx">
                                    <swiper-item @click.stop="activityJumping2" :data-item="item" :data-id="item.id">
                                        ...
                                        <view v-if="counterSwiperObj[item.grabActivityIdList[item.grabActivityIdList.length - 1]].grabData1.products && counterSwiperObj[item.grabActivityIdList[item.grabActivityIdList.length - 1]].grabData1.products[itemNum + 1]">
                                            <image lazy-load="" mode="aspectFit" :src="counterSwiperObj[item.grabActivityIdList[item.grabActivityIdList.length - 1]].grabData1.products[itemNum + 1].imageUrl" style="height: 100%;"></image>
                                        </view>
                                    </swiper-item>
                                </view>
                            </swiper>
             ...
             <!-- 三分图 -->
            <view v-if="item.prdouctDisplayStyle === 'THREE' && item.imageItems.length > 0">
              <view class="activity-img three-img" :style="'width: 100%;padding: 0 ' + (pageMargin || 0) + 'px;height:' + (item.imageItems[0].imageHeight || 0) + 'rpx;box-sizing: border-box'">
                <view v-for="(item , idx) in item.imageItems" :key="idx">
                  <view v-if="idx == 0" :style="'overflow: hidden;width: calc(50% - ' + (modelMargin ? modelMargin * 2 : 2) + 'px);height: calc(100% - ' + (modelMargin ? modelMargin * 2 : 2) + 'px);margin: ' + modelMargin + 'px;border-radius: ' + fillet + 'px'">
                    <image lazy-load="" :src="item.imageUrl" :data-id="item.id" @click="activityJumping2" :data-item="item" style="height: 100%;"></image>
                    <view class="swiper_activity_box" v-if="item.linkType == 'URL' && item.slideshowProduct == 'Y'">
                        <swiper @click.stop="activityJumping2" :data-item="item" :data-id="item.id" :style="'padding: ' + (item.spacingT || 0) + 'px ' + (item.spacingR || 0) + 'px ' + (item.spacingB || 0) + 'px ' + (item.spacingL || 0) + 'px'" :circular="true" :vertical="item.direction == 'about' ? false : true" :autoplay="true" :interval="3000">
                            <view v-for="(itemNum , idx) in urlSwiperObj[item.id].numberList" :key="idx">
                                <swiper-item @click.stop="activityJumping2" :data-item="item" :data-id="item.id">
                  ...
                  <view class="rightbox" :style="'overflow: hidden;width: calc(50% - ' + (modelMargin ? modelMargin * 2 : 2) + 'px);height: calc(100% - ' + (modelMargin ? modelMargin * 2 : 2) + 'px);margin: ' + modelMargin + 'px;'">
                    <view v-for="(item , idx) in item.imageItems" :key="idx">
                      <view :style="'overflow: hidden;height: calc(50% - ' + (middleMargin ? middleMargin / 2 : 0) + 'px);margin-bottom: ' + (idx == 1 ? middleMargin : 0) + 'px;border-radius: ' + fillet + 'px'" v-if="idx != 0">
                          <image lazy-load="" :src="item.imageUrl" mode="widthFix" :data-id="item.id" @click="activityJumping2" :data-item="item"></image>
                          <view class="swiper_activity_box" v-if="item.linkType == 'URL' && item.slideshowProduct == 'Y'">
                              <swiper @click.stop="activityJumping2" :data-item="item" :data-id="item.id" :style="'padding: ' + (item.spacingT || 0) + 'px ' + (item.spacingR || 0) + 'px ' + (item.spacingB || 0) + 'px ' + (item.spacingL || 0) + 'px'" :circular="true" :vertical="item.direction == 'about' ? false : true" :autoplay="true" :interval="3000">
                                  <view v-for="(itemNum , idx) in urlSwiperObj[item.id].numberList" :key="idx">
                                      <swiper-item @click.stop="activityJumping2" :data-item="item" :data-id="item.id">
            ...
            <view v-if="item.productsForPc.length > 0">
                <!-- 横向滑动 -->
                <scroll-view class="module-goods" scroll-x="" :style="'width: 100%;padding: 0 ' + (item.pageMargin || 0) + 'px;box-sizing: border-box'" v-if="item.prdouctDisplayStyle === 'LATERALS_SLIP'">
                    <view :style="'display: flex;flex-wrap: wrap;width: calc(' + (item.pageMargin ? item.pageMargin * 2 : 0) + 'px + ' + (item.rowNumber || item.productsForPcLength || 8) * 260 + 'rpx)'">
                        <view v-for="(goodsItem , index) in item.productsForPc" :key="idx">
                            <goods-item-small :activityColor="item.activityColor" :backGroundColor="item.backGroundColor" :backGroundType="item.backGroundType" :backGroundImgObj="item.backGroundImgObj" :goodsWidth="'260rpx'" :goods="goodsItem" :fillet="item.fillet" :shopCarType="item.shopCarType" :underlinePriceShow="item.underlinePriceShow" :styleType="item.styleType" :textStyle="item.textStyle" :alignStyle="item.alignStyle" :modelMargin="item.modelMargin" style="display: inline-block;margin-right: 0;" :shopCart="shopCart" :hasUserInfo="hasUserInfo" @onClickGoods="onClickGoods" @addToCart="addToCart" @noop="noop" @getUserInfo="onGetUserInfo" :idx="1"></goods-item-small>
                        </view>
                    </view>
                </scroll-view>
                <!-- 列表 -->
                <view class="module-goods" v-if="item.prdouctDisplayStyle === 'LIST'" :style="'width: 100%;padding: 0 ' + (item.pageMargin || 0) + 'px;box-sizing: border-box'">
                    <view v-for="(goodsItem , index) in item.productsForPc" :key="idx">
                        <goods-item :activityColor="item.activityColor" :backGroundColor="item.backGroundColor" :backGroundType="item.backGroundType" :backGroundImgObj="item.backGroundImgObj" :goods="goodsItem" :fillet="item.fillet" :styleType="item.styleType" :shopCarType="item.shopCarType" :underlinePriceShow="item.underlinePriceShow" :textStyle="item.textStyle" :alignStyle="item.alignStyle" :modelMargin="item.modelMargin" :shopCart="shopCart" :hasUserInfo="hasUserInfo" @onClickGoods="onClickGoods" @addToCart="addToCart" @noop="noop" @getUserInfo="onGetUserInfo"></goods-item>
                    </view>
                </view>
                <!-- 一行两个 -->
    ...
    <view v-if="item.type === 'GRAB'">
        <view class="index-grab" :style="'margin: ' + moduleSpacing + 'px 0'" v-if="grabData.products.length > 0">
            ...
            <scroll-view class="grab-goods-list" scroll-x="" style="width: 100%" v-if="grabData.products.length > 0">
            ...
            <goods-item-small goods="{{item}}" hasUserInfo="{{hasUserInfo}}" bind:onClickGoods="onClickGoods" bind:addToCart="addToCart" bind:noop="noop" bind:getUserInfo="onGetUserInfo"></goods-item-small>
                ...
                <view class="module-goods-item" v-for="(item , index) in grabData.products" :key="index" :data-activityId="item.activityId" :data-productId="item.productId" :id="'products-' + item.productId" data-type="secondkill" @click="toGoodsDetail">
                    <view class="module-goods-img">
                        <image lazy-load="" :src="item.imageUrl" mode="widthFix"></image>
                        <image lazy-load="" class="no-balance" mode="widthFix" :src="imagesPath.soldOutIcon" v-if="(item.storeBalance <= 0 || item.balance <= 0) && item.business !== 'DISTRIBUTION' && item.status !== 'OFF'"></image>
                        
                        <image lazy-load="" class="no-balance" mode="widthFix" :src="imagesPath.iconLowerShelf" v-if="item.status === 'OFF'"></image>
                    </view>
              ...
              <view class="module-goods-labels">
                <view wx:for="{{item.labels}}" wx:key="index" data-id="{{index}}" wx:if="{{index < 3}}" style="border-color:rgb({{item.color}});color:{{item.color}}">{{item.labelDetails}}</view>
                        ...
                        <view class="module-goods-price">
                            <text class="nowPrice"><text style="font-size: 24rpx;">￥</text>{{item.price ? item.price : ''}}<text style="font-size: 24rpx;" v-if="goods.style == 'SPEC_PARENT'">起</text></text>
                            <text class="oldPrice" v-if="item.originalPrice"><text style="font-size: 24rpx;">￥</text>{{item.originalPrice}}<text style="font-size: 24rpx;" v-if="goods.style == 'SPEC_PARENT'">起</text></text>
                        </view>
...
<view class="theme-more" v-if="totalCount.length >= 10" :style="'color:' + theme.color" @click="loadMore">查看更多>>
</view>
</view>
...
// import signin from '@/pages-sub/newretail/components/signin/signin.vue';
const app = getApp();
...
watch(() => props.pageScrollData, (newVal, oldVal) => {
...
watch(() => props.itemData, (newVal, oldVal) => {
  ...
  if (!newVal || newVal.id == state.upDataObj.id && state.loading || newDateTime < state.updateTime + 2000 && state.updateTime != 0) {
    return false;
  }
  state.upDataObj = newVal;
  state.loading = true;
  state.updateTime = newDateTime;
  ...
  const arr = [...newVal.imageItems];
  const typeObj = {
    'erfentu': 'TWO_PER_LINE',
    'sanfentu': 'THREE',
    'yihangsanzhang': 'THREE_PER_LINE',
    'yihangsizhang': 'TIVE_PER_LINE',
    'henghualiebiao': 'LATERALS_SLIP'
  };
  newVal.prdouctDisplayStyle = typeObj[newVal.type] || newVal.prdouctDisplayStyle;
  arr.map(item => {
  ...
  newVal.imageItems.forEach(async arr => {
    const arrImages = [];
    const arrImages1 = [];
    if (arr.linkType == 'URL' && arr.linkUrl && arr.slideshowProduct == 'Y') {
      ...
      if (detailsForAdsenseId) {
        await activityService.getDetailsForAdsenseId(detailsForAdsenseId).then(detailsForAdsense => {
          const detailsForAdsenseContent = detailsForAdsense ? JSON.parse(detailsForAdsense.content) : {};
    ...
    // 首页获取配置的秒杀活动商品轮播图
    if (arr.linkType != 'URL' && arr.appReturnType == 'MIAOSHA' && arr.grabActivityIdList && arr.grabActivityIdList.length > 0) {
      if (state.storeId || app.globalData.storeInfo && app.globalData.storeInfo.id) {
        // 判断是否存在门店id，不存在接口会报错
        queryGrabActivity1(state.storeId || app.globalData.storeInfo.id, arr.grabActivityIdList ? arr.grabActivityIdList[arr.grabActivityIdList.length - 1] : null);
      }
    }
    if (arrImages && arrImages.length > 0) {
      ...
      const urlSwiperObj = uni.getStorageSync('urlSwiperObj') ? JSON.parse(uni.getStorageSync('urlSwiperObj')) : state.urlSwiperObj;
      urlSwiperObj[arr.id] = {
        numberList: arr.numberList,
        arrImages: arr.arrImages
      };
      state.urlSwiperObj = urlSwiperObj;
      uni.setStorageSync('urlSwiperObj', JSON.stringify(state.urlSwiperObj));
      state.loading = false;
    } else {
      if (arr.linkType != 'URL' && arr.appReturnType == 'MIAOSHA' && arr.grabActivityIdList && arr.grabActivityIdList.length > 0) {} else {
        state.loading = false;
      }
    }
  });
...
function reload(refresh = false) {
  ...
  if (!state.loaded) {
    ...
    lazyLoadBase.determineComponentInVisibleArea(".theme-activities-loading", that).then(res => {
      if (res) {
        state.loaded = true;
        if (state.storeId !== "" || app.globalData.storeInfo && app.globalData.storeInfo.id) {
          getIndexActivity(state.storeId);
...
function getIndexActivity(storeId) {
  ...
  state.hasQuery = true;
  activityService.getActivityByStoreId(storeId).then(res => {
      ...
      res.records.forEach(item => {
        ...
        if (!flag) {
          ...
          if (item.type === 'TEAMBUYING' && GROUP === false) {
            ...
            //           console.log(tempData,imageItem.grabActivityIdList, 'counterSwiperObjcounterSwiperObj')
            //             that.queryGrabActivity1(storeId,imageItem.grabActivityIdList ? imageItem.grabActivityIdList[imageItem.grabActivityIdList.length - 1]: null)
            //         }
            //     })
            // }
...
function handleQueryGrabActivity1(storeIdIn, activeId11) {
  ...
  storeIdIn = storeIdIn ? storeIdIn.filter(item => item != '') : [];
  const postData = {
    ...
    if (res && res.length > 0) {
      ...
      for (let i = 0; i < allGrabList.length; i++) {
        if (i === 0 && allGrabList[i].countDown >= 0) {
          ...
          if (allGrabList[i].countDown >= 0 && tempDuration > allGrabList[i].countDown && allGrabList[i].grabStatus === 'started') {
            tempDuration = allGrabList[i].countDown;
            key = i;
          }
        }
      ...
      seckillService.queryProduct(postData).then(res => {
        ...
        tempGrabData.products.forEach((product, index) => {
          ...
          if (newList.length >= tempGrabData.products.length) {
            ...
            newList = newList.length % 2 == 1 ? [...newList, ...newList] : newList;
            ...
            // })
            const counterSwiperObj = uni.getStorageSync('counterSwiperObj') ? JSON.parse(uni.getStorageSync('counterSwiperObj')) : state.counterSwiperObj;
...
function handleGetBargainActivity(storeIdIn) {
  ...
  bargainService.query(postData).then(res => {
    const tempList = [];
...
function toBargainDetails(e) {
  if (e.detail && e.detail.id) {
    ...
    if (app.globalData.userInfo && app.globalData.userInfo.member) {
      ...
      bargainService.queryInstance(postData).then(res => {
        if (!res || !res.records || res.records.length === 0) {
...
function queryIndexActivity(storeId, page, pageSize) {
  ...
  activityService.queryPage(page, pageSize, opts).then(res => {
    inQueryHomeActivity = false;
