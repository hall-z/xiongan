...
<view class="push-collec-block" :hidden="!isSearchFixed"></view>
<view class="push-collec-pop" :hidden="isPushCollectShowed" @click.stop="toHidePushCollect">
  <image lazy-load="" :style="'top:' + navHeight + 'px;'" class="push-collect-board" mode="widthFix" :src="imagesPath.pushCollect"></image>
</view>
</template>
<script setup>
...
function reload(refresh = false) {
  const that = this;
  if (!state.loaded) {
    lazyLoadBase.determineComponentInVisibleArea(".push-collect-loading", that).then(res => {
      if (res) {
        state.loaded = true;
      }
    }).catch(err => {
