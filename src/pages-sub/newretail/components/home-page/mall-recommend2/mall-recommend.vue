...
function getScorll() {
  ...
  lazyLoadBase
    .determineComponentInVisibleArea(
      `.gm-small-goods-img-box${state.fineQualityGoodsList && state.fineQualityGoodsList.length ? state.fineQualityGoodsList.length - 1 : ''}`,
      that,
      `.mall-recommend-good`,
      500,
    )
...
function getStoreProduct(page) {
  ...
  const storeIdList = [state.storeId]
      ...
      if (res && res.records && res.records.length > 0) {
        ...
        function getGoodList() {
          ...
          state.goodTimeOut = setTimeout(() => {
            index++
            const fineQualityGoodsList = [
              ...state.fineQualityGoodsList,
              ...newTempList.slice(index * 50, (index + 1) * 50),
            ]
