  ...
  >
    ...
    <view class="px-10px">
      <template v-for="(item, index) in dataList" :key="index">
        <view
          class="p-10px bg-#f7f7f7 rounded-5px mb-10px flex items-center gap-10px"
          v-show="item.position === tab"
        >
          <view class="">
            <wd-img
              :src="JSON.parse(item.scenicImg)[0].url"
              width="94"
              height="100"
              mode="aspectFill"
            ></wd-img>
