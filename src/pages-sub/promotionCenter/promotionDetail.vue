...
// 根据当前用户id 查询推广人信息
useRequest(() => getPromoterUserByUserId<activationResult>(), {
  immediate: true,
  loading: false,
}).onSuccess((res) => {
  if (res.data.id) {
...
const { send: sedGetCountStatus, data: CountStatusData } = useRequest(
  () => getCountStatus<IGetCountStatusData>({ promoterId: promoterId.value }),
  {
    immediate: false,
    loading: false,
    initialData: {
  ...
  <view class="dy-blue-deep-bg">
    ...
    <view class="">
      <view class="promoDetailBox">
        <Promotion-Successful ref="successfulRef" title="推广码" :type="2" :id="promoterId" />
