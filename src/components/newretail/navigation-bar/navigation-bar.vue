<template>
  ...
  <weui-navigation-bar
    ...
    :color="props.color"
    :background="
      props.isHome && props.isShowTopNavigationFlag && !props.isShowTopNavigation
        ? 'transparent'
        : props.background
    "
    :title="props.title"
      ...
      <template v-slot:left>
        <view>
          <view :class="'left-box ' + (!props.isShowTopNavigationFlag ? 'top-left-box' : '')">
            <!-- <view class="address" wx:if="{{nearStoreStyle === '1'}}" bindtap='changeStore'> -->
            <view class="address" v-if="props.nearStoreStyle === '1'" @click="changeAddress">
              <image
                style="width: 22rpx; height: 30rpx"
                :src="state.imagesPath.iconNearStoreAddress"
              ></image>
              <!-- <text>{{storeInfoName || '暂无门店信息'}}</text> -->
              <text>{{ state.addressName || '暂无地址信息' }}</text>
            </view>
