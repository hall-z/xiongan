...
const canvasIdBar = ref('canvasIdBar')
// #ifndef MP-WEIXIN || MP-ALIPAY || MP-QQ
canvasIdBar.value = 'dy' + uuid()
// #endif
...
function appvueH5Other() {
  return Promise.resolve(uni.createCanvasContext(canvasIdBar.value, vnodeCtx))
}
// 支付宝和微信，QQ 支持2d渲染。
...
function drawNvue_init() {
  /* 获取元素引用 */
  const ganvas = vnodeCtx.$refs[canvasIdBar.value]
  /* 通过元素引用获取canvas对象 */
